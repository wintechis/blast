...
d=h(d);p.setCatchFinallyBlocks(3);console.log("Writing to "+g.uuid);return p.yield(g.writeValueWithoutResponse(d),5);case 5:console.log("Wrote to "+g.uuid);p.leaveTryBlock(0);break;case 3:m=p.enterCatchBlock(),console.error(m),Blast.throwError("Error writing to Bluetooth device.\nMake sure the device is compatible with the connected block."),p.jumpToEnd()}})};
Blast.Bluetooth.getPrimaryService=function(a,b){var c,d,h;return $jscomp.asyncExecutePromiseGeneratorProgram(function(g){switch(g.nextAddress){case 1:return g.yield(Blast.Bluetooth.connect(a),2);case 2:return c=g.yieldResult,console.log("connected to"),console.log(c),g.setCatchFinallyBlocks(3),console.log("Getting primary service "+b+" from "+a),g.yield(c.getPrimaryService(b),5);case 5:d=g.yieldResult;console.log("Got primary service "+b+" from "+a);g.leaveTryBlock(4);break;case 3:h=g.enterCatchBlock(),
console.error(h),Blast.throwError("The device is not compatible with the connected block.");case 4:return g.return(d)}})};
