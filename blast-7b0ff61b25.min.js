...
a),console.error(d),f.jumpToEnd()}})};
Blast.Bluetooth.gatt_writeWithoutResponse=function(a,b,c,d){var f,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){switch(h.nextAddress){case 1:return h.yield(Blast.Bluetooth.getCharacteristic(a,b,c),2);case 2:f=h.yieldResult;if(!f)return h.return();"string"===typeof d&&(d=hexStringToArrayBuffer(d));h.setCatchFinallyBlocks(3);Blast.Ui.addToLog("Invoke WriteValueWithoutResponse on characteristic <code>"+c+"</code> with value <code>"+(d.toString()+"</code>"),"Bluetooth",a);return h.yield(f.writeValueWithoutResponse(d),
5);case 5:Blast.Ui.addToLog("Finished WriteValueWithoutResponse on characteristic <code>"+c+"</code> with value <code>"+(d.toString()+"</code>"),"Bluetooth",a);h.leaveTryBlock(0);break;case 3:g=h.enterCatchBlock(),console.error(g),Blast.throwError("Error writing to Bluetooth device.\nMake sure the device is compatible with the connected block."),h.jumpToEnd()}})};
Blast.Bluetooth.gatt_writeWithResponse=function(a,b,c,d){var f,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){switch(h.nextAddress){case 1:return h.yield(Blast.Bluetooth.getCharacteristic(a,b,c),2);case 2:f=h.yieldResult;if(!f)return h.return();"string"===typeof d&&(d=hexStringToArrayBuffer(d));console.log(d);h.setCatchFinallyBlocks(3);Blast.Ui.addToLog("Invoke WriteValueWithResponse on characteristic <code>"+c+"</code> with value <code>"+(d.toString()+"</code>"),"Bluetooth",a);
return h.yield(f.writeValueWithResponse(d),5);case 5:Blast.Ui.addToLog("Finished WriteValueWithResponse on characteristic <code>"+c+"</code> with value <code>"+(d.toString()+"</code>"),"Bluetooth",a);h.leaveTryBlock(0);break;case 3:g=h.enterCatchBlock(),console.error(g),Blast.throwError("Error writing to Bluetooth device.\nMake sure the device is compatible with the connected block."),h.jumpToEnd()}})};
...
Blast.Bluetooth.gatt_read_number=function(a,b,c){var d,f,g,h,m;return $jscomp.asyncExecutePromiseGeneratorProgram(function(k){if(1==k.nextAddress)return k.yield(Blast.Bluetooth.gatt_read(a,b,c),2);d=k.yieldResult;d instanceof DataView||(d=new DataView(value));f=new Uint8Array(d.buffer);g=f.length;h=buffer.Buffer.from(f);m=h.readUIntBE(0,g);return k.return(m)})};
Blast.Bluetooth.gatt_read_hex=function(a,b,c){var d,f;return $jscomp.asyncExecutePromiseGeneratorProgram(function(g){if(1==g.nextAddress)return g.yield(Blast.Bluetooth.gatt_read(a,b,c),2);d=g.yieldResult;f=(new Uint8Array(d.buffer)).reduce(function(h,m){return h+("0"+m.toString(16)).slice(-2)},"");return g.return(f)})};
...
b(f),m.return(h.namespace+h.instance);case "10":return h=c(f),m.return(h.url);case "20":return h=d(f),m.return(h.beaconTemperature);case "30":return Blast.throwError("EID frame type is not supported by blast."),m.return();default:return Blast.throwError("Eddystone frame type is not valid."),m.return()}})};
Blast.Bluetooth.Eddystone.setAdvertisingData=function(a,b){var c,d,f;return $jscomp.asyncExecutePromiseGeneratorProgram(function(g){if(1==g.nextAddress)return Blast.Ui.addToLog("Set Eddystone advertising data...","Eddystone",a),c=function(h){for(var m=["http://www.","https://www.","http://","https://"],k=".com/ .org/ .edu/ .net/ .info/ .biz/ .gov/ .com .org .edu .net .info .biz .gov".split(" "),l=[],n=0,p=new TextEncoder("utf-8"),q=0;q<m.length;q++)if(h.startsWith(m[q])){l.push(q);n=m[q].length;break}for(;n<
