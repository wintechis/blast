 ...
 */
export const getStatus = function (): string {
  return status;
};
 ...
 */
export const intervalEvents: number[] = [];
 ...
 */
let stdErr: (text: string) => void = console.error;

/**
 * Setter for the Interpreter's standard error output function.
 * @public
 */
export const setStdError = function (fn: (text: string) => void) {
  stdErr = fn;
};
 ...
 */
export const getStdError = function (): (text: string) => void {
  return stdErr;
};
 ...
 */
let interpreterExecutionExit = false;
...
export const runJS = async function () {
  ...
  try {
    ...
    const func = new AsyncFunction(
      'f',
      `${latestCode}
      if(${eventsInWorkspace.length === 0}) {f()};`
    );
    await func(stopJS);
  } catch (e) {
    stopJS();
