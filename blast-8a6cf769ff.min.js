...
var sendHttpRequest=function(a,b,c,d,f,h){var g,n,k,l,m;return $jscomp.asyncExecutePromiseGeneratorProgram(function(p){switch(p.nextAddress){case 1:return null!=a&&void 0!=a&&""!=a||Blast.throwError("URI input of HttpRequest blocks must not be empty"),g=JSON.parse(c),n={method:b,headers:new Headers(g)},"GET"!==b&&d&&(n.body=d),p.setCatchFinallyBlocks(2),p.yield(fetch(a,n),4);case 4:k=p.yieldResult;if(!k.ok)return Blast.throwError("Failed to get "+a+", Error: "+k.status+" "+k.statusText),p.return();
"status"==f&&h(k.status);return p.yield(k.text(),5);case 5:l=p.yieldResult;h(l);p.leaveTryBlock(0);break;case 2:m=p.enterCatchBlock(),Blast.throwError("Failed to get "+a+", Error: "+m.message),p.jumpToEnd()}})};Blast.asyncApiFunctions.push(["sendHttpRequest",sendHttpRequest]);
