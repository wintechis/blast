...
Blast.runJS=function(){Blast.resetUi(Blast.status.RUNNING);Blast.Ui.addMessage("execution started","info");null==Blast.Interpreter&&(Blast.highlightPause=!1,Blast.Interpreter=new Interpreter(Blast.latestCode,initApi),Blast.runner_=function(){if(Blast.Interpreter)try{Blast.Interrupted?setTimeout(Blast.runner_,1):Blast.Interpreter.step()?setTimeout(Blast.runner_,5):Blast.States.Interpreter||0<Blast.eventInWorkspace.length||(Blast.resetUi(Blast.status.READY),Blast.Ui.addMessage("execution completed",
...
if("things_webBluetooth"==f.$jscomp$loop$prop$type$75||"things_webHID"==f.$jscomp$loop$prop$type$75){f.$jscomp$loop$prop$name$76=f.$jscomp$loop$prop$block$77.firstElementChild.textContent;if(d.includes(f.$jscomp$loop$prop$name$76))continue;h=document.createElement("tr");var g=document.createElement("td");g.appendChild(document.createTextNode(f.$jscomp$loop$prop$name$76));var m=document.createElement("td"),k=document.createElement("input");k.setAttribute("type","button");k.setAttribute("id","pairButton-"+
f.$jscomp$loop$prop$name$76);k.setAttribute("value","Pair");var l=document.createElement("td"),n=document.createElement("span");n.setAttribute("id","pairStatus-"+f.$jscomp$loop$prop$name$76);n.innerHTML="&#x2718;";n.style.color="red";n.style.fontSize="20px";n.style.fontWeight="bold";n.style.marginRight="10px";l.appendChild(n);k.addEventListener("click",function(p){return function(){var q,r;return $jscomp.asyncExecutePromiseGeneratorProgram(function(t){if(1==t.nextAddress)return"things_webBluetooth"!=
...
g.setAttribute("aria-hidden","true");g.innerHTML="things_webBluetooth"==f.$jscomp$loop$prop$type$79?"bluetooth_disabled":"usb_off";h.appendChild(g);g=document.createElement("span");g.setAttribute("class","mdc-list-item__text");var m=document.createElement("span");m.setAttribute("class","mdc-list-item__primary-text");m.innerHTML=f.$jscomp$loop$prop$name$80;g.appendChild(m);m=document.createElement("span");m.setAttribute("class","mdc-list-item__secondary-text");m.setAttribute("id","rc-status-"+f.$jscomp$loop$prop$name$80);
...
"connected";document.getElementById("rc-icon-"+k.$jscomp$loop$prop$name$80).innerHTML="bluetooth";document.getElementById("rc-icon-"+k.$jscomp$loop$prop$name$80).style.color="#0d30b1";k.$jscomp$loop$prop$block$81.firstElementChild.textContent=n.id;Blast.Storage.allConnectedMobile_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").getAttribute("data-hasEvent")||(document.getElementById("rc-done").setAttribute("data-hasEvent",!0),document.getElementById("rc-done").addEventListener("click",
...
Blast.Bluetooth.Eddystone.getAdvertisingData=function(a){var b,c,d,f,h,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(m){if(1==m.nextAddress)return Blast.Ui.addToLog("Reading Eddystone advertising data...","Eddystone",a),b=function(k){var l=parseInt(k.substring(2,4),16),n=k.substring(4,24);k=k.substring(25,37);return{frameType:"UID",txPower:l,namespace:n,instance:k}},c=function(k){var l=parseInt(k.substring(2,4),16);switch(k.substring(4,6)){case "00":var n="http://www.";break;case "01":n=
"https://www.";break;case "02":n="http://";break;case "03":n="https://";break;default:Blast.throwError("Eddystone URL scheme is not valid.");return}var p=".com/ .org/ .edu/ .net/ .info/ .biz/ .gov/ .com .org .edu .net .info .biz .gov".split(" ");k=k.substring(6,k.length);for(var q=0;q<k.length;q+=2){var r=parseInt(k.substring(q,q+2),16);n=r<p.length?n+p[r]:n+String.fromCharCode(r)}return{frameType:"URL",txPower:l,url:n}},d=function(k){var l=parseInt(k.substring(4,8),16),n=parseInt(k.substring(8,10),
16)+parseInt(k.substring(10,12),16)/100,p=parseInt(k.substring(12,20),16);k=parseInt(k.substring(20,28),16);return{frameType:"TLM",batteryVoltage:l,beaconTemperature:n,pduCount:p,timeSinceReboot:k}},m.yield(Blast.Bluetooth.gatt_read_hex(a,Blast.Bluetooth.Eddystone.UUIDS.CONFIG_SERVICE,Blast.Bluetooth.Eddystone.UUIDS.ADV_SLOT_DATA_CHARACTERISTIC),2);f=m.yieldResult;Blast.Ui.addToLog("Got Eddystone advertising data: <code>"+f+"</code>","Eddystone",a);h=f.substring(0,2);switch(h){case "00":return g=
...
for(d=c.next();!d.done;d=c.next())d=d.value,a.setProperty(b,d[0],a.createAsyncFunction(d[1]));c=function(f,h){return Blast.States.eventChecker(f,h)};a.setProperty(b,"eventChecker",a.createNativeFunction(c));a.setProperty(b,"startEventChecker",a.createNativeFunction(function(){return Blast.States.startEventChecker()}));c=function(f){return prompt(f)};a.setProperty(b,"prompt",a.createNativeFunction(c))};Blockly.JavaScript.repeat=Blockly.JavaScript.controls_repeat_ext;Blockly.JavaScript.while_until=Blockly.JavaScript.controls_whileUntil;Blockly.JavaScript["for"]=Blockly.JavaScript.controls_for;Blockly.JavaScript.break_continue=Blockly.JavaScript.controls_flow_statements;Blockly.JavaScript.conditional_statement=Blockly.JavaScript.controls_if;Blockly.JavaScript.wait_seconds=function(a){return"waitForSeconds("+(Blockly.JavaScript.valueToCode(a,"SECONDS",Blockly.JavaScript.ORDER_ATOMIC)||0)+");\n"};
...
Blockly.Blocks.capture_image={init:function(){this.appendDummyInput().appendField("capture image from camera");this.setOutput(!0,"String");this.setColour(0);this.setTooltip("Captures an image from a camera.");this.setHelpUrl("")},onChange:function(){navigator.mediaDevices&&navigator.mediaDevices.getUserMedia||this.setWarningText("Your browser does not support mediaDevices.")}};Blast.Toolbox.addBlock("capture_image","Actions");
...
var sendHttpRequest=function(a,b,c,d,f,h){var g,m,k,l,n;return $jscomp.asyncExecutePromiseGeneratorProgram(function(p){switch(p.nextAddress){case 1:return null!=a&&void 0!=a&&""!=a||Blast.throwError("URI input of HttpRequest blocks must not be empty"),g=JSON.parse(c),m={method:b,headers:new Headers(g)},d&&(m.body=d),p.setCatchFinallyBlocks(2),p.yield(fetch(a,m),4);case 4:k=p.yieldResult;if(!k.ok)return Blast.throwError("Failed to get "+a+", Error: "+k.status+" "+k.statusText),p.return();"status"==
f&&h(k.status);return p.yield(k.text(),5);case 5:l=p.yieldResult;h(l);p.leaveTryBlock(0);break;case 2:n=p.enterCatchBlock(),Blast.throwError("Failed to get "+a+", Error: "+n.message),p.jumpToEnd()}})};Blast.asyncApiFunctions.push(["sendHttpRequest",sendHttpRequest]);
...
var captureImage=function(a){var b,c,d,f,h,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(m){if(1==m.nextAddress)return console.log("captureImage"),b=document.createElement("video"),b.id="video",b.setAttribute("autoplay","autoplay"),b.setAttribute("muted",!0),b.style.display="none",document.body.appendChild(b),c={video:!0},m.yield(navigator.mediaDevices.getUserMedia(c),2);if(3!=m.nextAddress)return d=m.yieldResult,b.srcObject=d,console.log(b),f=document.createElement("canvas"),h=f.getContext("2d"),
m.yield(new Promise(function(k,l){b.onloadedmetadata=function(){f.width=b.videoWidth;f.height=b.videoHeight;h.drawImage(b,0,0,f.width,f.height);k()}}),3);g=f.toDataURL("image/png");console.log(g);b.srcObject.getTracks().forEach(function(k){return k.stop()});b.remove();f.remove();a(g);return m.return("")})};Blast.asyncApiFunctions.push(["captureImage",captureImage]);
...
return $jscomp.asyncExecutePromiseGeneratorProgram(function(q){p=n.detail;if(!p||!p.actualOrientation)return q.return();Blast.Ui.addToLog("Received <code>hidinput</code> event from Joy-Con: <code>"+JSON.stringify(p)+"</code>","hid",g.productname);Blast.Ui.addToLog("Removing <code>hidinput</code> event listener","hid",g.productname);m.removeEventListener("hidinput",k);""!==d?"accelerometers"===b?h(p[b][c][d].acc):"gyroscopes"===b?h(p[b][c][d][f]):h(p[b][c][d]):h(p[b][c]);q.jumpToEnd()})};if(m.eventListenerAttached){l.jumpTo(0);
break}return l.yield(m.open(),10);case 10:return l.yield(m.enableStandardFullMode(),11);case 11:return l.yield(m.enableIMUMode(),12);case 12:return l.yield(m.enableVibration(),13);case 13:Blast.Ui.addToLog("Adding <code>hidinput</code> event listener","hid",g.productname),m.addEventListener("hidinput",k),m.eventListenerAttached=!0,l.jumpToEnd()}})};Blast.asyncApiFunctions.push(["readJoyConProperty",readJoyConProperty]);
...
p.leaveTryBlock(3);break;case 2:g=p.enterCatchBlock();if("InvalidStateError"!=g.name)return Blast.throwError(g),d(),p.return();f.close();return p.yield(StreamDeck.openDevice(f),6);case 6:h=p.yieldResult;case 3:m=parseInt(c.substring(1,3),16),k=parseInt(c.substring(3,5),16),l=parseInt(c.substring(5,7),16),n=0;case 7:if(!(n<b.length)){p.jumpTo(9);break}if("1"!==b.charAt(n)){p.jumpTo(8);break}Blast.Ui.addToLog("Invoke <code>fillKeyColor</code> with value <code>"+[n,m,k,l].toString()+"</code>","hid",
f.productName);return p.yield(h.fillKeyColor(n,m,k,l),11);case 11:Blast.Ui.addToLog("Finished <code>fillKeyColor</code>","hid",f.productName);case 8:n++;p.jumpTo(7);break;case 9:d(),p.jumpToEnd()}})};Blast.asyncApiFunctions.push(["streamdeckColorButtons",streamdeckColorButtons]);
...
8,60,.8*k.width);n=l.getImageData(0,0,k.width,k.height);for(p=0;p<b.length;p++)"1"===b.charAt(p)&&(Blast.Ui.addToLog("Invoke <code>fillKeyImageData</code> with value <code>"+[p,n].toString()+"</code>","hid",f.productName),m.push(h.fillKeyBuffer(p,buffer.Buffer.from(n.data),{format:"rgba"})),Blast.Ui.addToLog("Finished <code>fillKeyImageData</code>","hid",f.productName));l.restore();return q.yield(Promise.all(m),7);case 7:d(),q.jumpToEnd()}})};
