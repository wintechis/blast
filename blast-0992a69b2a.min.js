...
b);m=[];for(p=0;p<g.byteLength;p++)m.push("0x"+("00"+g.getUint8(p).toString(16)).slice(-2));l=d(m);c(l[a]);n.jumpToEnd()})};Blast.asyncApiFunctions.push(["getRuuviProperty",getRuuviProperty]);Blockly.Blocks.streamdeck_buttons={init:function(){var a=this;this.appendValueInput("id").setCheck("Thing").appendField("Streamdeck mini");this.appendDummyInput().appendField("on button press:");this.appendDummyInput().setAlign(Blockly.ALIGN_CENTRE).appendField(new Blockly.FieldCheckbox("FALSE",function(b){return a.uncheckAllOtherCheckboxes(b,"button1")}),"button1").appendField(new Blockly.FieldCheckbox("FALSE",function(b){return a.uncheckAllOtherCheckboxes(b,"button2")}),"button2").appendField(new Blockly.FieldCheckbox("FALSE",
function(b){return a.uncheckAllOtherCheckboxes(b,"button3")}),"button3");this.appendDummyInput().setAlign(Blockly.ALIGN_CENTRE).appendField(new Blockly.FieldCheckbox("FALSE",function(b){return a.uncheckAllOtherCheckboxes(b,"button4")}),"button4").appendField(new Blockly.FieldCheckbox("FALSE",function(b){return a.uncheckAllOtherCheckboxes(b,"button5")}),"button5").appendField(new Blockly.FieldCheckbox("FALSE",function(b){return a.uncheckAllOtherCheckboxes(b,"button6")}),"button6");this.appendDummyInput().appendField("do");
...
onchange:function(){this.isInFlyout||this.requested||!this.rendered||this.addEvent()},onDispose:function(a){a.type===Blockly.Events.BLOCK_DELETE&&a.type===Blockly.Events.BLOCK_DELETE&&-1!==a.ids.indexOf(this.id)&&this.removeFromEvents()},removeFromEvents:function(){var a=Blast.eventInWorkspace.indexOf(this.id);-1!==a&&Blast.eventInWorkspace.splice(a,1)}};Blast.Toolbox.addBlock("streamdeck_buttons","States and Events");Blockly.JavaScript.streamdeck_buttons=function(a){var b="TRUE"==a.getFieldValue("button1")?1:0,c="TRUE"==a.getFieldValue("button2")?1:0,d="TRUE"==a.getFieldValue("button3")?1:0,h="TRUE"==a.getFieldValue("button4")?1:0,g="TRUE"==a.getFieldValue("button5")?1:0,m="TRUE"==a.getFieldValue("button6")?1:0,p=Blockly.JavaScript.valueToCode(a,"id",Blockly.JavaScript.ORDER_NONE)||null;b=Blockly.JavaScript.quote_(""+b+c+d+h+g+m);a=Blockly.JavaScript.quote_(Blockly.JavaScript.statementToCode(a,"statements"));
Blockly.JavaScript.definitions_.eventHandlers=(Blockly.JavaScript.definitions_.eventHandlers||"")+("handleStreamdeck("+p+", "+b+", "+a+");\n");return""};
var handleStreamdeck=function(a,b,c,d){var h,g,m,p,l;return $jscomp.asyncExecutePromiseGeneratorProgram(function(n){switch(n.nextAddress){case 1:if(null===a)return Blast.throwError("No streamdeck block set."),d(),n.return();h=Blast.Things.webHidDevices.get(a);if(!h)return Blast.throwError("Connected device is not a HID device.\nMake sure you are connecting the Streamdeck via webHID"),d(),n.return();n.setCatchFinallyBlocks(2);return n.yield(StreamDeck.openDevice(h),4);case 4:g=n.yieldResult;n.leaveTryBlock(3);
break;case 2:if(m=n.enterCatchBlock(),"InvalidStateError"!==m.name)return Blast.throwError(m),d(),n.return();case 3:for(l=0;l<b.length;l++)if(1===b.indexOf(l)){p=l;break}if(void 0===p)return d(),n.return();console.log(p);g.on("down",function(q){if(q===p){Blast.Interrupted=!0;var t=new Interpreter("");t.stateStack[0].scope=Blast.Interpreter.globalScope;t.appendCode(c);var w=function(){try{t.step()?setTimeout(w,5):Blast.Interrupted=!1}catch(z){Blast.throwError("Error executing program:\n "+e),console.error(z)}};
w()}});n.jumpToEnd()}})};Blast.asyncApiFunctions.push(["handleStreamdeck",handleStreamdeck]);/*
