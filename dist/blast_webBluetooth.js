...
/**
   ...
   */
export const writeWithoutResponse = async function (id, serviceUUID, characteristicUUID, value) {
    const characteristic = await getCharacteristic(id, serviceUUID, characteristicUUID);
    if (!characteristic) {
        return;
    }
    ...
    try {
        const thingsLog = getThingsLog();
        thingsLog('Invoke <code>WriteValueWithoutResponse</code> on characteristic ' +
            `<code>${characteristicUUID}</code> with value <code>${value.toString()}</code>`, 'Bluetooth', id);
        await characteristic.writeValueWithoutResponse(value);
        thingsLog('Finished <code>WriteValueWithoutResponse</code> on characteristic ' +
            `<code>${characteristicUUID}</code> with value <code>${value.toString()}</code>`, 'Bluetooth', id);
    }
    catch (error) {
...
/**
   ...
   * @return {Object} representation of the complete request with response.
   */
export const writeWithResponse = async function (id, serviceUUID, characteristicUUID, value) {
    const characteristic = await getCharacteristic(id, serviceUUID, characteristicUUID);
    if (!characteristic) {
        return;
    }
    ...
    try {
        const thingsLog = getThingsLog();
        thingsLog('Invoke <code>WriteValueWithResponse</code> on characteristic ' +
            `<code>${characteristicUUID}</code> with value <code>${value.toString()}</code>`, 'Bluetooth', id);
        await characteristic.writeValueWithResponse(value);
        thingsLog('Finished <code>WriteValueWithResponse</code> on characteristic ' +
            `<code>${characteristicUUID}</code> with value <code>${value.toString()}</code>`, 'Bluetooth', id);
    }
    catch (error) {
        const errorMsg = 'Error writing to Bluetooth device.\nMake sure the device is compatible with the connected block.';
...
export const getCharacteristic = async function (id, serviceUUID, characteristicUUID) {
    ...
    try {
        const thingsLog = getThingsLog();
        thingsLog(`Getting characteristic <code>${characteristicUUID}</code> from service <code>${serviceUUID}</code>`, 'Bluetooth', id);
        characteristic = await service.getCharacteristic(characteristicUUID);
        thingsLog(`Got characteristic <code>${characteristicUUID}</code> from service <code>${serviceUUID}</code>`, 'Bluetooth', id);
    }
    catch (error) {
        console.error(error);
...
export const read = async function (id, serviceUUID, characteristicUUID) {
    ...
    try {
        const thingsLog = getThingsLog();
        thingsLog(`Invoke <code>ReadValue</code> on characteristic <code>${characteristicUUID}</code>` +
            ` from service <code>${serviceUUID}</code>`, 'Bluetooth', id);
        const value = await characteristic.readValue();
        thingsLog(`Finished <code>ReadValue</code> on characteristic <code>${characteristicUUID}</code>` +
...
export const subscribe = async function (id, serviceUUID, charUUID, handler) {
    ...
    const thingsLog = getThingsLog();
    thingsLog(`Add 'characteristicvaluechanged' listener to characteristic ${charUUID}` +
    ...
    try {
        thingsLog(`Invoke <code>startNotifications</code> on characteristic ${charUUID}` +
            ` from service <code>${serviceUUID}</code>`, 'Bluetooth', id);
        await characteristic.startNotifications();
        thingsLog(`Finished <code>startNotifications</code> on characteristic ${charUUID}` +
            ` from service <code>${serviceUUID}</code>`, 'Bluetooth', id);
    }
    catch (error) {
        console.error(error);
...
export const startLEScan = async function () {
    ...
    Blockly.dialog.alert('Please click allow on the LE Scan prompt now, then close this dialog.', navigator.bluetooth.requestLEScan({ acceptAllAdvertisements: true, keepRepeatedDevices: true }));
...
//# sourceMappingURL=blast_webBluetooth.js.map