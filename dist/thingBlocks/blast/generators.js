...
Blockly.JavaScript['http_request'] = function (block) {
    const uri = Blockly.JavaScript.valueToCode(block, 'uri', Blockly.JavaScript.ORDER_NONE) || null;
    const method = block.getFieldValue('METHOD');
    const headers = Blockly.JavaScript.valueToCode(block, 'header', Blockly.JavaScript.ORDER_NONE);
    const output = block.getFieldValue('OUTPUT');
    const body = Blockly.JavaScript.valueToCode(block, 'body', Blockly.JavaScript.ORDER_NONE);
 ...
 * @private
 */
const sendHttpRequest = async function (uri, method, headersString, body, output, callback) {
...
Blockly.JavaScript['display_text'] = function (block) {
    const message = Blockly.JavaScript.valueToCode(block, 'text', Blockly.JavaScript.ORDER_NONE) || '\'\'';
...
const playAudio = async function (uri, callback) {
    await new Promise((resolve, reject) => {
        ...
        audio.autoplay = true;
        audio.onerror = ((error) => {
            throwError(`Error trying to play audio from \n${uri}\n See console for details`);
            console.error(error);
            reject(error);
        });
