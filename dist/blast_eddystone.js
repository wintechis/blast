...
const getCapabilities = async function (webBluetoothId) {
    ...
    let capabilitiesArray = await read(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.CAPABILITIES_CHARACTERISTIC);
...
export const getActiveSlot = async function (webBluetoothId) {
    ...
    const activeSlot = await readNumber(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.ACTIVE_SLOT_CHARACTERISTIC);
...
export const setActiveSlot = async function (webBluetoothId, slot) {
    ...
    if (slot < 0 || slot >= capabilities.maxSlots) {
        throwError(`Eddystone slot is not valid.
         On this device the slot must be between 0 and ${capabilities.maxSlots - 1}.`);
    }
    await writeWithResponse(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.ACTIVE_SLOT_CHARACTERISTIC, slot.toString(16));
...
export const getAdvertisingInterval = async function (webBluetoothId) {
    ...
    const interval = await readNumber(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.ADVERTISING_INTERVAL_CHARACTERISTIC);
...
export const setAdvertisingInterval = async function (webBluetoothId, interval) {
    ...
    await writeWithResponse(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.ADVERTISING_INTERVAL_CHARACTERISTIC, interval.toString(16));
...
export const getTxPowerLevel = async function (webBluetoothId) {
    ...
    const txPowerLevel = await readNumber(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.RADIO_TX_POWER_CHARACTERISTIC);
...
export const setTxPowerLevel = async function (webBluetoothId, txPowerLevel) {
    ...
    if (capabilities.supportedTxPowerLevels.indexOf(txPowerLevel) === -1) {
        throwError(`Eddystone TX power level is not valid.
         On this device the TX power level must be one of ${capabilities.supportedTxPowerLevels}.`);
        return;
    }
    await writeWithResponse(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.RADIO_TX_POWER_CHARACTERISTIC, txPowerLevel.toString(16));
...
export const getAdvertisedTxPower = async function (webBluetoothId) {
    ...
    const advertisedTxPower = await readNumber(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.ADVERTISED_TX_POWER_CHARACTERISTIC);
...
export const setAdvertisedTxPower = async function (webBluetoothId, txPowerLevel) {
    ...
    await writeWithResponse(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.ADVERTISED_TX_POWER_CHARACTERISTIC, txPowerLevel.toString(16));
...
export const getLockState = async function (webBluetoothId) {
    ...
    const lockState = await readNumber(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.LOCK_STATE_CHARACTERISTIC);
...
export const getPublicECDHKey = async function (webBluetoothId) {
    ...
    const publicECDHKey = await readHex(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.PUBLIC_ECDH_KEY_CHARACTERISTIC);
...
export const getAdvertisingData = async function (webBluetoothId) {
    ...
    const advertisingData = await readHex(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.ADV_SLOT_DATA_CHARACTERISTIC);
...
export const setAdvertisingData = async function (webBluetoothId, data) {
    ...
    const response = await writeWithResponse(webBluetoothId, UUIDS.CONFIG_SERVICE, UUIDS.ADV_SLOT_DATA_CHARACTERISTIC, encodedData);
...
/**
   ...
   * @param {JSInterpreter.AsyncCallback} callback JS Interpreter callback.
   */
export const writeEddystoneProperty = async function (webBluetoothId, property, value) {
    switch (property) {
        case 'advertisedTxPower':
...
//# sourceMappingURL=blast_eddystone.js.map