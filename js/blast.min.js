...
function(){return Blast.Storage.reconnectDoneHandler_(a)}))}).catch(function(p){wHidLog("Argh! "+p)})}}(e));l.appendChild(g);h.appendChild(f);h.appendChild(l);h.appendChild(k);c.appendChild(h);d.push(e.$jscomp$loop$prop$name$39)}document.getElementById("rc-cancel").addEventListener("click",function(){return Blast.Storage.reconnectCancelHandler_()})}};
...
(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)}))}).catch(function(k){Blast.throwError("Error connecting to WebBluetooth device:\n"+k)}):"things_webHID"==g.$jscomp$loop$prop$type$41&&navigator.hid.requestDevice({filters:[]}).then(function(k){0===k.length&&Blast.throwError("Connection failed or cancelled by User.");var m=Date.now().toString(36)+Math.random().toString(36).substr(2);
Blast.Things.webHidDevices.set(m,k[0]);Blast.Things.addWebHidDevice(m,"");document.getElementById("rc-status-"+g.$jscomp$loop$prop$name$42).innerHTML="connected";document.getElementById("rc-icon-"+g.$jscomp$loop$prop$name$42).innerHTML="usb";document.getElementById("rc-icon-"+g.$jscomp$loop$prop$name$42).style.color="#0d30b1";Blast.Storage.allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)}))}).catch(function(k){wHidLog("Argh! "+
