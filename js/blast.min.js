...
Blast.Storage.removeDeviceId_=function(a){var b=a.querySelectorAll("block");b=$jscomp.makeIterator(b);for(var c=b.next();!c.done;c=b.next())c=c.value,"things_webBluetooth"==c.getAttribute("type")&&(c=c.firstElementChild)&&c.remove();return a};Blast.Storage.saveXML_=function(a,b){Blockly.hideChaff();fetch(a,{method:"PUT",body:b}).then(function(c){c.ok?(location.hash=a,Blockly.alert(Blast.Storage.LINK_ALERT.replace("%1",window.location.href))):Blast.throwError(Blast.Storage.HTTPREQUEST_ERROR)})};
Blast.Storage.load=function(){var a=document.getElementById("loadWorkspace-input").value;""==a?Blockly.alert("Enter a URI first."):Blast.Storage.retrieveXML_(a)};
