...
$jscomp.polyfill("Array.prototype.findIndex",function(a){return a?a:function(b,c){return $jscomp.findInternal(this,b,c).i}},"es6","es3");$jscomp.polyfill("Array.prototype.values",function(a){return a?a:function(){return $jscomp.iteratorFromArray(this,function(b,c){return c})}},"es8","es3");
...
$jscomp.polyfill("Array.from",function(a){return a?a:function(b,c,d){c=null!=c?c:function(m){return m};var f=[],g="undefined"!=typeof Symbol&&Symbol.iterator&&b[Symbol.iterator];if("function"==typeof g){b=g.call(b);for(var h=0;!(g=b.next()).done;)f.push(c.call(d,g.value,h++))}else for(g=b.length,h=0;h<g;h++)f.push(c.call(d,b[h],h));return f}},"es6","es3");var Blast={States:{}};Blast.States.Interpreter=null;Blast.States.runner_=null;Blast.States.latestCode="";Blast.States.eventCode=new Map;
...
Blast.init=function(){if(!window.location.href.includes("mobile")){var a=document.getElementById("content_area"),b=function(){for(var c=Blast.getBBox_(a),d=$jscomp.makeIterator(Blast.Ui.TABS_),f=d.next();!f.done;f=d.next())f=document.getElementById("content_"+f.value),f.style.top=c.y+"px",f.style.left=c.x+"px",f.style.height=c.height+"px",f.style.height=2*c.height-f.offsetHeight+"px",f.style.width=c.width+"px",f.style.width=2*c.width-f.offsetWidth+"px";Blast.workspace&&Blast.workspace.getToolbox().width&&
...
Blast.generateCode=function(){Blockly.JavaScript.STATEMENT_PREFIX="highlightBlock(%1);\n";Blockly.JavaScript.addReservedWords("highlightBlock");Blast.latestCode="";Blast.latestCode=Blockly.JavaScript.workspaceToCode(Blast.workspace)};Blast.resetInterpreter=function(){Blast.Interpreter=null;Blast.runner_&&(clearTimeout(Blast.runner_),Blast.runner_=null);Blast.Bluetooth.tearDown();Blast.removeDeviceHandlers()};
Blast.removeDeviceHandlers=function(){for(var a=$jscomp.makeIterator(Blast.deviceEventHandlers),b=a.next();!b.done;b=a.next()){b=b.value;var c=b.device;console.log(c);b=c.removeEventListener(b.type,b.fn);console.log(b)}Blast.deviceEventHandlers=[]};
...
console.log(u)})),t.jumpTo(0)):t.yield(Blast.Bluetooth.requestDevice(),3);r=t.yieldResult;document.getElementById("pairStatus-"+p.$jscomp$loop$prop$name$51).innerHTML="&#x2714;";document.getElementById("pairStatus-"+p.$jscomp$loop$prop$name$51).style.color="green";p.$jscomp$loop$prop$block$52.firstElementChild.textContent=r.id;Blast.Storage.allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").getAttribute("data-hasEvent")||(document.getElementById("rc-done").setAttribute("data-hasEvent",
...
(document.getElementById("rc-done").setAttribute("data-hasEvent",!0),document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)})))}).catch(function(q){Blast.throwError("Connection failed or cancelled by User.");console.log(q)})),p.jumpTo(0)):p.yield(Blast.Bluetooth.requestDevice(),3);n=p.yieldResult;Blast.Things.addWebBluetoothDevice(n.id,k.$jscomp$loop$prop$name$55);document.getElementById("rc-status-"+k.$jscomp$loop$prop$name$55).innerHTML=
...
a),console.error(g),h.jumpToEnd()}})};Blast.Bluetooth.startLEScan=function(){return $jscomp.asyncExecutePromiseGeneratorProgram(function(a){if(Blast.Bluetooth.isLEScanRunning)return a.return();Blast.Bluetooth.cacheLEScanResults();Blast.Bluetooth.isLEScanRunning=!0;Blockly.alert("Please click allow on the LE Scan prompt now, then close this dialog.",navigator.bluetooth.requestLEScan({acceptAllAdvertisements:!0,keepRepeatedDevices:!0}));setTimeout(Blast.Bluetooth.stopLEScan,3E4);a.jumpToEnd()})};
...
return null};Blockly.JavaScript.uri=function(a){return[Blockly.JavaScript.quote_(a.getFieldValue("URI")),Blockly.JavaScript.ORDER_NONE]};Blockly.JavaScript.mac=function(a){return[Blockly.JavaScript.quote_(a.getFieldValue("MAC")),Blockly.JavaScript.ORDER_NONE]};/*
 https://www.gnu.org/licenses/agpl-3.0.html AGPLv3
*/
Blockly.JavaScript.finish=function(a){var b=Blockly.utils.object.values(Blockly.JavaScript.definitions_);a=Object.getPrototypeOf(Blockly.JavaScript).finish.call(Blockly.JavaScript,a);Blockly.JavaScript.isInitialized=!1;Blockly.JavaScript.nameDB_?Blockly.JavaScript.nameDB_.reset():Blockly.JavaScript.nameDB_=new Blockly.Names(Blockly.JavaScript.RESERVED_WORDS_);return b.join("\n\n")+"\n\n\n"+a};Blockly.JavaScript.number_value=Blockly.JavaScript.math_number;Blockly.JavaScript.number_infinity=function(a){return[Infinity,Blockly.JavaScript.ORDER_NONE]};
...
var getRSSIWb=function(a,b){var c,d,f,g,h;return $jscomp.asyncExecutePromiseGeneratorProgram(function(m){if(1==m.nextAddress)return m.yield(navigator.bluetooth.getDevices(),2);c=m.yieldResult;d=null;f=$jscomp.makeIterator(c);for(g=f.next();!g.done;g=f.next())if(h=g.value,h.id===a){d=h;break}null==d&&Blast.throwError("Error pairing with Bluetooth device.");d.addEventListener("advertisementreceived",function(k){return $jscomp.asyncExecutePromiseGeneratorProgram(function(l){b(k.rssi);l.jumpToEnd()})});
return m.yield(d.watchAdvertisements(),0)})};Blast.asyncApiFunctions.push(["getRSSIWb",getRSSIWb]);Blockly.Blocks.switch_lights_rgb={init:function(){this.appendValueInput("red").setCheck("Number").appendField("write color properties: red ");this.appendValueInput("green").setCheck("Number").appendField("green");this.appendValueInput("blue").setCheck("Number").appendField("blue");this.appendValueInput("thing").setCheck("Thing").appendField("to LED controller");this.setInputsInline(!0);this.setPreviousStatement(!0,null);this.setNextStatement(!0,null);this.setColour(255);this.setTooltip("Switches checked lights on and unchecked ones off.");
