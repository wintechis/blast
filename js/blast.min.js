...
if("things_webBluetooth"==e.$jscomp$loop$prop$type$43||"things_webHID"==e.$jscomp$loop$prop$type$43){e.$jscomp$loop$prop$name$44=e.$jscomp$loop$prop$block$45.firstElementChild.textContent;if(d.includes(e.$jscomp$loop$prop$name$44))continue;g=document.createElement("tr");var f=document.createElement("td");f.appendChild(document.createTextNode(e.$jscomp$loop$prop$name$44));var l=document.createElement("td"),h=document.createElement("input");h.setAttribute("type","button");h.setAttribute("id","pairButton-"+
e.$jscomp$loop$prop$name$44);h.setAttribute("value","Pair");var k=document.createElement("td"),m=document.createElement("span");m.setAttribute("id","pairStatus-"+e.$jscomp$loop$prop$name$44);m.innerHTML="&#x2718;";m.style.color="red";m.style.fontSize="20px";m.style.fontWeight="bold";m.style.marginRight="10px";k.appendChild(m);h.addEventListener("click",function(n){return function(){var p,q;return $jscomp.asyncExecutePromiseGeneratorProgram(function(r){if(1==r.nextAddress)return"things_webBluetooth"!=
...
"&#x2714;";document.getElementById("pairStatus-"+n.$jscomp$loop$prop$name$44).style.color="green";n.$jscomp$loop$prop$block$45.firstElementChild.textContent=q.id;Blast.Storage.allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)}));r.jumpToEnd()})}}(e));l.appendChild(h);g.appendChild(f);g.appendChild(l);g.appendChild(k);c.appendChild(g);d.push(e.$jscomp$loop$prop$name$44)}document.getElementById("rc-cancel").addEventListener("click",
...
l.innerHTML="disconnected";f.appendChild(l);g.appendChild(f);g.addEventListener("click",function(h){return function(){var k,m;return $jscomp.asyncExecutePromiseGeneratorProgram(function(n){if(1==n.nextAddress)return"things_webBluetooth"!=h.$jscomp$loop$prop$type$47?("things_webHID"==h.$jscomp$loop$prop$type$47&&(k=[],navigator.hid.requestDevice({filters:k}).then(function(p){0===p.length&&Blast.throwError("Connection failed or cancelled by User.");var q=Date.now().toString(36)+Math.random().toString(36).substr(2);
Blast.Things.webHidDevices.set(q,p[0]);Blast.Things.addWebHidDevice(q,h.$jscomp$loop$prop$name$48);document.getElementById("rc-status-"+h.$jscomp$loop$prop$name$48).innerHTML="connected";document.getElementById("rc-icon-"+h.$jscomp$loop$prop$name$48).innerHTML="usb";document.getElementById("rc-icon-"+h.$jscomp$loop$prop$name$48).style.color="#0d30b1";h.$jscomp$loop$prop$block$49.firstElementChild.textContent=q;Blast.Storage.allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").addEventListener("click",
function(){return Blast.Storage.reconnectDoneHandler_(a)}))}).catch(function(p){wHidLog("Argh! "+p)})),n.jumpTo(0)):n.yield(Blast.Bluetooth.requestDevice(),3);m=n.yieldResult;Blast.Things.addWebBluetoothDevice(m.id,h.$jscomp$loop$prop$name$48);document.getElementById("rc-status-"+h.$jscomp$loop$prop$name$48).innerHTML="connected";document.getElementById("rc-icon-"+h.$jscomp$loop$prop$name$48).innerHTML="bluetooth";document.getElementById("rc-icon-"+h.$jscomp$loop$prop$name$48).style.color="#0d30b1";
h.$jscomp$loop$prop$block$49.firstElementChild.textContent=m.id;Blast.Storage.allConnectedMobile_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)}));n.jumpToEnd()})}}(e));c.appendChild(g);d.push(e.$jscomp$loop$prop$name$48)}window.app.initLists()};
...
Blast.Bluetooth.gatt_subscribe=function(a,b,c,d){var e,g,f;return $jscomp.asyncExecutePromiseGeneratorProgram(function(l){switch(l.nextAddress){case 1:return l.yield(Blast.Bluetooth.connect(a,b),2);case 2:return e=l.yieldResult,l.yield(e.getPrimaryService(b),3);case 3:return g=l.yieldResult,l.yield(g.getCharacteristic(c),4);case 4:return f=l.yieldResult,f.addEventListener("characteristicvaluechanged",d),Blast.Bluetooth.charEventListeners[c]=[f,"characteristicvaluechanged",d],l.yield(f.startNotifications(),
0)}})};Blast.Bluetooth.startLEScan=function(){return $jscomp.asyncExecutePromiseGeneratorProgram(function(a){if(Blast.Bluetooth.isLEScanRunning)return a.return();Blast.Bluetooth.cacheLEScanResults();Blast.Bluetooth.isLEScanRunning=!0;Blockly.alert("Please click allow on the LE Scan prompt now, then close this dialog.",navigator.bluetooth.requestLEScan({acceptAllAdvertisements:!0,keepRepeatedDevices:!0}));setTimeout(Blast.Bluetooth.stopLEScan,3E4);a.jumpToEnd()})};
...
Blast.Bluetooth.tearDown=function(){Blast.Bluetooth.isLEScanRunning&&(Blast.Bluetooth.isLEScanRunning=!1);if(Blast.Bluetooth.eventListeners)for(var a=$jscomp.makeIterator(Blast.Bluetooth.eventListeners),b=a.next();!b.done;b=a.next())b=b.value,Blast.Bluetooth.removeEventListener(b[0],b[1]);if(Blast.Bluetooth.charEventListeners)for(a=$jscomp.makeIterator(Object.keys(Blast.Bluetooth.charEventListeners)),b=a.next();!b.done;b=a.next())b=Blast.Bluetooth.charEventListeners[b.value],b[0].removeEventListener(b[1],
b[2]);Blast.Bluetooth.LEScanResults={}};function initApi(a,b){Blockly.JavaScript.addReservedWords("highlightBlock","eventChecker");var c=function(e){e=e?e.toString():"";return Blast.highlightBlock(e)};a.setProperty(b,"highlightBlock",a.createNativeFunction(c));c=function(e){Blast.Interrupted=e};a.setProperty(b,"setInterrupt",a.createNativeFunction(c));c=$jscomp.makeIterator(Blast.apiFunctions);for(var d=c.next();!d.done;d=c.next())d=d.value,a.setProperty(b,d[0],a.createNativeFunction(d[1]));c=$jscomp.makeIterator(Blast.asyncApiFunctions);
...
var getRSSIWb=function(a,b){var c,d,e,g,f;return $jscomp.asyncExecutePromiseGeneratorProgram(function(l){if(1==l.nextAddress)return l.yield(navigator.bluetooth.getDevices(),2);c=l.yieldResult;d=null;e=$jscomp.makeIterator(c);for(g=e.next();!g.done;g=e.next())if(f=g.value,f.id===a){d=f;break}null==d&&Blast.throwError("Error pairing with Bluetooth device.");d.addEventListener("advertisementreceived",function(h){return $jscomp.asyncExecutePromiseGeneratorProgram(function(k){b(h.rssi);k.jumpToEnd()})});
...
var readMijiaProperty=function(a,b,c){b?Blast.Bluetooth.gatt_subscribe(b,XiaomiServiceUUID,"ebe0ccc1-7a0a-4b0c-8a1a-6ff2997da3a6",function(d){console.log(d);var e=d.target.value,g=e.getUint8(1)&128;d=(e.getUint8(1)&127)<<8|e.getUint8(0);g&&(d-=32767);d/=100;e=e.getUint8(2);"temperature"===a?c(d):"humidity"===a&&c(e)}):(Blast.throwError("No Thermometer is set."),c())};Blast.asyncApiFunctions.push(["readMijiaProperty",readMijiaProperty]);
