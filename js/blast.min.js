...
Blast.Ui.init=function(){Blast.Ui.uriInput=document.getElementById("loadWorkspace-input");Blast.Ui.runButton=document.getElementById("runButton");Blast.Ui.messageOutputContainer=document.getElementById("msgOutputContainer");Blast.Ui.statusContainer=document.getElementById("statusContainer");Blast.Ui.tabClick_(Blast.Ui.selected_);for(var a=$jscomp.makeIterator(Blast.Ui.TABS_),b=a.next();!b.done;b=a.next())b=b.value,Blast.bindClick("tab_"+b,function(c){return function(){Blast.Ui.tabClick_(c)}}(b));
window.location.href.includes("mobile.html")||(Blast.bindClick("loadButton",Blast.Storage.load),Blast.bindClick("saveButton",Blast.Storage.link),Blast.Ui.uriInput.addEventListener("keyup",function(c){13===c.keyCode&&Blast.Storage.load()}))};Blast.Toolbox={};
...
Blast.States.generateCode(),Blast.Ui.renderContent_())});Blast.Ui.addMessage("Actionblock output will be displayed here");window.setTimeout(Blast.importPrettify,1)}};Blast.highlightBlock=function(a){Blast.workspace.highlightBlock(a);Blast.highlightPause=!0};Blast.resetUi=function(a){Blast.workspace.highlightBlock(null);Blast.highlightPause=!1;Blast.Ui.setStatus(a)};
