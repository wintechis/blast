...
$jscomp.polyfill("Array.prototype.keys",function(a){return a?a:function(){return $jscomp.iteratorFromArray(this,function(b){return b})}},"es6","es3");$jscomp.polyfill("Array.prototype.findIndex",function(a){return a?a:function(b,c){return $jscomp.findInternal(this,b,c).i}},"es6","es3");
...
Blast.init=function(){var a=document.getElementById("content_area"),b=function(){for(var c=Blast.getBBox_(a),d=$jscomp.makeIterator(Blast.Ui.TABS_),e=d.next();!e.done;e=d.next())e=document.getElementById("content_"+e.value),e.style.top=c.y+"px",e.style.left=c.x+"px",e.style.height=c.height+"px",e.style.height=2*c.height-e.offsetHeight+"px",e.style.width=c.width+"px",e.style.width=2*c.width-e.offsetWidth+"px";Blast.workspace&&Blast.workspace.getToolbox().width&&(document.getElementById("tab_workspace").style.minWidth=
Blast.workspace.getToolbox().width-38+"px")};window.addEventListener("resize",b,!1);Blast.workspace=Blockly.inject("content_workspace",{media:"media/",toolbox:Blast.Toolbox.currentToolbox,zoom:{controls:!0,wheel:!0}});Blast.Toolbox.init();b();Blockly.svgResize(Blast.workspace);Blast.Ui.init();Blast.Ui.setStatus(Blast.status.READY);Blast.generateCode();Blast.workspace.addChangeListener(function(c){c instanceof Blockly.Events.Ui||(Blast.generateCode(),Blast.States.generateCode(),Blast.Ui.renderContent_())});
Blast.Ui.addMessage("Actionblock output will be displayed here");window.setTimeout(Blast.importPrettify,1)};Blast.highlightBlock=function(a){Blast.workspace.highlightBlock(a);Blast.highlightPause=!0};Blast.resetUi=function(a){Blast.workspace.highlightBlock(null);Blast.highlightPause=!1;Blast.Ui.setStatus(a)};Blast.generateCode=function(){Blockly.JavaScript.STATEMENT_PREFIX="highlightBlock(%1);\n";Blockly.JavaScript.addReservedWords("highlightBlock");Blast.latestCode="";Blast.latestCode=Blockly.JavaScript.workspaceToCode(Blast.workspace)};
Blast.resetInterpreter=function(){Blast.Interpreter=null;Blast.runner_&&(clearTimeout(Blast.runner_),Blast.runner_=null);Blast.Bluetooth.tearDown()};Blast.removeDeviceHandlers=function(){for(var a=$jscomp.makeIterator(Blast.deviceEventHandlers),b=a.next();!b.done;b=a.next())b=b.value,b.device.removeEventListener(b.type,b.fn);Blast.deviceEventHandlers=[]};
...
Blast.Storage.removeDeviceId_=function(a){var b=a.querySelectorAll("block");b=$jscomp.makeIterator(b);for(var c=b.next();!c.done;c=b.next())if(c=c.value,"things_webBluetooth"==c.getAttribute("type")&&(c=c.firstElementChild)){for(var d=c.textContent,e=void 0,k=$jscomp.makeIterator(Blast.Things.webBluetoothDevices.entries()),f=k.next();!f.done;f=k.next()){f=$jscomp.makeIterator(f.value);var l=f.next().value;if(f.next().value===d){e=l;break}}c.textContent=e}return a};
...
Blast.Storage.retrieveXML_=function(a){return $jscomp.asyncExecutePromiseGeneratorProgram(function(b){Blockly.hideChaff();Blast.resetInterpreter();Blast.resetUi(Blast.status.READY);fetch(a).then(function(c){if(c.ok)return c.text();Blast.throwError(Blast.Storage.NOT_FOUND_ERROR.replace("%1",a))}).then(function(c){Blast.workspace.clear();try{c=Blockly.Xml.textToDom(c)}catch(d){Blast.throwError(Blast.Storage.XML_ERROR+"\nXML: "+c);return}c.querySelector('block[type="things_webBluetooth"]')?(Blast.Storage.generatePairButtons(c),
document.getElementById("rcModal").style.display="block"):(Blockly.Xml.domToWorkspace(c,Blast.workspace),Blast.Storage.monitorChanges_(Blast.workspace))});b.jumpToEnd()})};
Blast.Storage.generatePairButtons=function(a){for(var b=a.querySelectorAll("block"),c=document.getElementById("rc-tbody");c.firstChild;)c.removeChild(c.firstChild);var d={};b=$jscomp.makeIterator(b);for(var e=b.next();!e.done;d={$jscomp$loop$prop$name$36:d.$jscomp$loop$prop$name$36},e=b.next())if(e=e.value,"things_webBluetooth"==e.getAttribute("type")){d.$jscomp$loop$prop$name$36=e.firstElementChild.textContent;e=document.createElement("tr");var k=document.createElement("td");k.appendChild(document.createTextNode(d.$jscomp$loop$prop$name$36));
var f=document.createElement("td"),l=document.createElement("input");l.setAttribute("type","button");l.setAttribute("id","pairButton-"+d.$jscomp$loop$prop$name$36);l.setAttribute("value","Pair");var g=document.createElement("td"),h=document.createElement("span");h.setAttribute("id","pairStatus-"+d.$jscomp$loop$prop$name$36);h.innerHTML="&#x2718;";h.style.color="red";h.style.fontSize="20px";h.style.fontWeight="bold";h.style.marginRight="10px";g.appendChild(h);l.addEventListener("click",function(m){return function(){navigator.bluetooth.requestDevice({acceptAllDevices:!0,
optionalServices:["0000fff0-0000-1000-8000-00805f9b34fb"]}).then(function(n){Blast.Things.addWebBluetoothDevice(n.id,m.$jscomp$loop$prop$name$36);document.getElementById("pairStatus-"+m.$jscomp$loop$prop$name$36).innerHTML="&#x2714;";document.getElementById("pairStatus-"+m.$jscomp$loop$prop$name$36).style.color="green";Blast.Storage.allConnected_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)}))}).catch(function(n){Blast.throwError("Error connecting to WebBluetooth device:\n"+
n)})}}(d));f.appendChild(l);e.appendChild(k);e.appendChild(f);e.appendChild(g);c.appendChild(e);document.getElementById("rc-cancel").addEventListener("click",function(){return Blast.Storage.reconnectCancelHandler_()})}};Blast.Storage.allConnected_=function(){var a=document.getElementById("rc-tbody").querySelectorAll("tr");a=$jscomp.makeIterator(a);for(var b=a.next();!b.done;b=a.next())if("&#x2718;"==document.getElementById("pairStatus-"+b.value.firstElementChild.textContent).innerHTML)return!1;return!0};
Blast.Storage.reconnectDoneHandler_=function(a){document.getElementById("rcModal").style.display="none";Blockly.Xml.domToWorkspace(a,Blast.workspace);Blast.Storage.monitorChanges_(Blast.workspace)};Blast.Storage.reconnectCancelHandler_=function(){document.getElementById("rcModal").style.display="none"};
...
b),2);case 2:return k=g.yieldResult,g.yield(k.getPrimaryService(b),3);case 3:return f=g.yieldResult,g.yield(f.getCharacteristic(c),4);case 4:return l=g.yieldResult,d=e(d),g.yield(l.writeValueWithoutResponse(d),0)}})};
Blast.Bluetooth.gatt_read=function(a,b,c){var d,e,k,f;return $jscomp.asyncExecutePromiseGeneratorProgram(function(l){switch(l.nextAddress){case 1:return l.setCatchFinallyBlocks(2),l.yield(Blast.Bluetooth.connect(a,b),4);case 4:return d=l.yieldResult,l.yield(d.getPrimaryService(b),5);case 5:return e=l.yieldResult,l.yield(e.getCharacteristic(c),6);case 6:return k=l.yieldResult,l.yield(k.readValue(),7);case 7:return l.return(l.yieldResult);case 2:f=l.enterCatchBlock(),Blast.throwError("Error reading from Bluetooth device "+
