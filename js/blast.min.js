...
g.firstElementChild.textContent;if(d.includes(e.$jscomp$loop$prop$name$44))continue;g=document.createElement("tr");var f=document.createElement("td");f.appendChild(document.createTextNode(e.$jscomp$loop$prop$name$44));var l=document.createElement("td"),h=document.createElement("input");h.setAttribute("type","button");h.setAttribute("id","pairButton-"+e.$jscomp$loop$prop$name$44);h.setAttribute("value","Pair");var k=document.createElement("td"),m=document.createElement("span");m.setAttribute("id",
"pairStatus-"+e.$jscomp$loop$prop$name$44);m.innerHTML="&#x2718;";m.style.color="red";m.style.fontSize="20px";m.style.fontWeight="bold";m.style.marginRight="10px";k.appendChild(m);h.addEventListener("click",function(n){return function(){var p;return $jscomp.asyncExecutePromiseGeneratorProgram(function(q){if(1==q.nextAddress)return"things_webBluetooth"!=n.$jscomp$loop$prop$type$43?("things_webHID"==n.$jscomp$loop$prop$type$43&&(p=[],navigator.hid.requestDevice({filters:p}).then(function(r){0===r.length&&
...
q.jumpToEnd()})}}(e));l.appendChild(h);g.appendChild(f);g.appendChild(l);g.appendChild(k);c.appendChild(g);d.push(e.$jscomp$loop$prop$name$44)}document.getElementById("rc-cancel").addEventListener("click",function(){return Blast.Storage.reconnectCancelHandler_()})}};
...
f=document.createElement("span");f.setAttribute("class","mdc-list-item__text");var l=document.createElement("span");l.setAttribute("class","mdc-list-item__primary-text");l.innerHTML=e.$jscomp$loop$prop$name$47;f.appendChild(l);l=document.createElement("span");l.setAttribute("class","mdc-list-item__secondary-text");l.setAttribute("id","rc-status-"+e.$jscomp$loop$prop$name$47);l.innerHTML="disconnected";f.appendChild(l);g.appendChild(f);g.addEventListener("click",function(h){return function(){var k;
return $jscomp.asyncExecutePromiseGeneratorProgram(function(m){if(1==m.nextAddress)return"things_webBluetooth"!=h.$jscomp$loop$prop$type$46?("things_webHID"==h.$jscomp$loop$prop$type$46&&(k=[],navigator.hid.requestDevice({filters:k}).then(function(n){0===n.length&&Blast.throwError("Connection failed or cancelled by User.");var p=Date.now().toString(36)+Math.random().toString(36).substr(2);Blast.Things.webHidDevices.set(p,n[0]);Blast.Things.addWebHidDevice(p,"");document.getElementById("rc-status-"+
...
var sendHttpRequest=function(a,b,c,d,e,g){var f,l;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){null!=a&&void 0!=a&&""!=a||Blast.throwError("URI input of HttpRequest blocks must not be empty");f=JSON.parse(c);l={method:b,headers:new Headers(f)};d&&(l.body=d);fetch(a,l).then(Blast.handleFetchErrors).then(function(k){"status"==e&&g(k.status);g(k.text())}).then(function(k){urdf.clear();urdf.load(k).then(function(){urdf.query("SELECT * WHERE {?subject ?predicate ?object}").then(function(m){g(m)})})}).catch(function(k){Blast.throwError(k.message+
...
b+"], "+a+");\n"};
var handleStreamdeck=function(a,b,c){var d,e,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(f){switch(f.nextAddress){case 1:d="inputreport";e=Blast.Things.webHidDevices.get(a);if(e.opened){f.jumpTo(2);break}f.setCatchFinallyBlocks(3);return f.yield(e.open(),5);case 5:f.leaveTryBlock(2);break;case 3:f.enterCatchBlock(),Blast.throwError("Failed to open device, your browser or OS probably doesn't support webHID.");case 2:g=function(l){if(1===l.reportId&&(l=new Int8Array(l.data.buffer),l=
Array.from(l).slice(0,6),b=b.toString().split(",").map(Number),l.every(function(m,n){return m===b[n]}))){Blast.Interrupted=!0;var h=new Interpreter("");h.stateStack[0].scope=Blast.Interpreter.globalScope;h.appendCode(c);var k=function(){try{h.step()?setTimeout(k,5):Blast.Interrupted=!1}catch(m){Blockly.alert("Error executing program:\n%e".replace("%e",m)),Blast.Ui.setStatus(Blast.status.ERROR),Blast.resetInterpreter(),console.error(m)}};k()}},Blast.deviceEventHandlers.push({device:e,type:d,fn:g}),
