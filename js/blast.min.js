...
Blast.Storage.removeDeviceId_=function(a){var b=a.querySelectorAll("block");b=$jscomp.makeIterator(b);for(var c=b.next();!c.done;c=b.next()){c=c.value;var d=c.getAttribute("type");if("things_webBluetooth"==d||"things_webHID"==d)if(c=c.firstElementChild){var e=c.textContent,h=void 0;d=$jscomp.makeIterator(("things_webBluetooth"==d?Blast.Things.webBluetoothDevices:Blast.Things.webHidDevices).entries());for(var f=d.next();!f.done;f=d.next()){f=$jscomp.makeIterator(f.value);var l=f.next().value;if(f.next().value===
e){h=l;break}}c.textContent=h}}return a};Blast.Storage.saveXML_=function(a,b){Blockly.hideChaff();fetch(a,{method:"PUT",body:b}).then(function(c){c.ok?(location.hash=a,Blockly.alert(Blast.Storage.LINK_ALERT.replace("%1",window.location.href))):Blast.throwError(Blast.Storage.HTTPREQUEST_ERROR)})};Blast.Storage.load=function(){var a=document.getElementById("loadWorkspace-input").value;""==a?Blockly.alert("Enter a URI first."):Blast.Storage.retrieveXML_(a)};
...
p.length&&Blast.throwError("Connection failed or cancelled by User.");wHidModal.style.display="none";var q=Date.now().toString(36)+Math.random().toString(36).substr(2);Blast.Things.webHidDevices.set(q,p[0]);Blast.Things.addWebHidDevice(q,"");document.getElementById("pairStatus-"+n.$jscomp$loop$prop$name$39).innerHTML="&#x2714;";document.getElementById("pairStatus-"+n.$jscomp$loop$prop$name$39).style.color="green";Blast.Storage.allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,
document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)}))}).catch(function(p){wHidLog("Argh! "+p)})}}(e));l.appendChild(g);h.appendChild(f);h.appendChild(l);h.appendChild(k);c.appendChild(h);d.push(e.$jscomp$loop$prop$name$39)}document.getElementById("rc-cancel").addEventListener("click",function(){return Blast.Storage.reconnectCancelHandler_()})}};
Blast.Storage.generatePairButtonsMobile_=function(a){for(var b=a.querySelectorAll("block"),c=document.getElementById("rc-list");c.firstChild;)c.removeChild(c.firstChild);var d=[],e={};b=$jscomp.makeIterator(b);for(var h=b.next();!h.done;e={$jscomp$loop$prop$name$41:e.$jscomp$loop$prop$name$41,$jscomp$loop$prop$type$42:e.$jscomp$loop$prop$type$42},h=b.next())if(h=h.value,e.$jscomp$loop$prop$type$42=h.getAttribute("type"),"things_webBluetooth"==e.$jscomp$loop$prop$type$42||"things_webHID"==e.$jscomp$loop$prop$type$42)if(e.$jscomp$loop$prop$name$41=
h.firstElementChild.textContent,!d.includes(e.$jscomp$loop$prop$name$41)){h=document.createElement("li");h.setAttribute("id","rc-item-"+e.$jscomp$loop$prop$name$41);h.setAttribute("class","mdc-list-item");var f=document.createElement("span");f.setAttribute("id","rc-icon-"+e.$jscomp$loop$prop$name$41);f.setAttribute("class","mdc-list-item__graphic material-icons");f.setAttribute("aria-hidden","true");f.innerHTML="things_webBluetooth"==e.$jscomp$loop$prop$type$42?"bluetooth_disabled":"usb_off";h.appendChild(f);
f=document.createElement("span");f.setAttribute("class","mdc-list-item__text");var l=document.createElement("span");l.setAttribute("class","mdc-list-item__primary-text");l.innerHTML=e.$jscomp$loop$prop$name$41;f.appendChild(l);l=document.createElement("span");l.setAttribute("class","mdc-list-item__secondary-text");l.setAttribute("id","rc-status-"+e.$jscomp$loop$prop$name$41);l.innerHTML="disconnected"==e.$jscomp$loop$prop$type$42;f.appendChild(l);h.appendChild(f);h.addEventListener("click",function(g){return function(){navigator.bluetooth.requestDevice({acceptAllDevices:!0,
optionalServices:["0000fff0-0000-1000-8000-00805f9b34fb"]}).then(function(k){Blast.Things.addWebBluetoothDevice(k.id,g.$jscomp$loop$prop$name$41);document.getElementById("rc-status-"+g.$jscomp$loop$prop$name$41).innerHTML="connected";document.getElementById("rc-icon-"+g.$jscomp$loop$prop$name$41).innerHTML="things_webBluetooth"==g.$jscomp$loop$prop$type$42?"bluetooth":"usb";document.getElementById("rc-icon-"+g.$jscomp$loop$prop$name$41).style.color="#0d30b1";Blast.Storage.allConnectedMobile_()&&(document.getElementById("rc-done").disabled=
!1,document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)}))}).catch(function(k){Blast.throwError("Error connecting to WebBluetooth device:\n"+k)})}}(e));c.appendChild(h)}window.app.initLists()};
