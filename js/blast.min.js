...
b),2);case 2:return h=g.yieldResult,g.yield(h.getPrimaryService(b),3);case 3:return f=g.yieldResult,g.yield(f.getCharacteristic(c),4);case 4:return l=g.yieldResult,d=e(d),g.yield(l.writeValueWithoutResponse(d),5);case 5:return g.return()}})};
Blast.Bluetooth.gatt_read=function(a,b,c){var d,e,h,f,l;return $jscomp.asyncExecutePromiseGeneratorProgram(function(g){switch(g.nextAddress){case 1:return g.setCatchFinallyBlocks(2),g.yield(Blast.Bluetooth.connect(a,b),4);case 4:return d=g.yieldResult,g.yield(d.getPrimaryService(b),5);case 5:return e=g.yieldResult,g.yield(e.getCharacteristic(c),6);case 6:return h=g.yieldResult,g.yield(h.readValue(),7);case 7:return f=g.yieldResult,g.return(f);case 2:l=g.enterCatchBlock(),Blast.throwError("Error reading from Bluetooth device "+
a),console.error(l),g.jumpToEnd()}})};function initApi(a,b){Blockly.JavaScript.addReservedWords("highlightBlock","eventChecker");var c=function(e){e=e?e.toString():"";return Blast.highlightBlock(e)};a.setProperty(b,"highlightBlock",a.createNativeFunction(c));c=function(e){Blast.Interrupted=e};a.setProperty(b,"setInterrupt",a.createNativeFunction(c));c=$jscomp.makeIterator(Blast.apiFunctions);for(var d=c.next();!d.done;d=c.next())d=d.value,a.setProperty(b,d[0],a.createNativeFunction(d[1]));c=$jscomp.makeIterator(Blast.asyncApiFunctions);
...
h.addEventListener(e,f),l.jumpToEnd()}})};Blast.asyncApiFunctions.push(["handleStreamdeck",handleStreamdeck]);Blockly.Blocks.get_ruuvi_measurement={init:function(){this.appendValueInput("Thing").setCheck("Thing").appendField("get").appendField(new Blockly.FieldDropdown([["temperature","temperature"],["humidity","humidity"],["pressure","pressure"],["acceleration X","accelerationX"],["acceleration Y","accelerationY"],["acceleration Z","accelerationZ"],["battery voltage","batteryVoltage"],["txPower","txPower"],["movement counter","movementCounter"],["measurement sequence counter","measurementSequenceCounter"]]),
"measurement").appendField("temperature of Ruuvi Tag");this.setOutput(!0,["String","Number"]);this.setColour(255);this.setTooltip("Reads the temperature property of a Ruuvi Tag.");this.setHelpUrl("");this.firstTime=!0},onchange:function(){!this.isInFlyout&&this.firstTime&&this.rendered&&(this.firstTime=!1,navigator.bluetooth||(Blockly.alert("Webbluetooth is not supported by this browser.\n\n        Upgrade to Chrome version 85 or later."),this.dispose()))}};
Blast.Toolbox.addBlock("get_ruuvi_measurement","Properties");Blockly.JavaScript.get_ruuvi_measurement=function(a){var b=a.getFieldValue("measurement");a=Blockly.JavaScript.valueToCode(a,"Thing",Blockly.JavaScript.ORDER_ATOMIC);return["getRuuviMmeasurement("+b+", "+a+")",Blockly.JavaScript.ORDER_NONE]};
var getRuuviMmeasurement=function(a,b,c){var d;return $jscomp.asyncExecutePromiseGeneratorProgram(function(e){if(1==e.nextAddress)return d=function(h){var f={},l=h[0]&255,g=h[1]<<8|h[2]&255;32767<g&&(g-=65535);var k=((h[3]&255)<<8|h[4]&255)/400,m=((h[5]&255)<<8|h[6]&255)+5E4,n=(h[7]<<8|h[8]&255)/1E3,p=(h[9]<<8|h[10]&255)/1E3,q=(h[11]<<8|h[12]&255)/1E3,r=(h[13]&255)<<8|h[14]&255,t=h[15]&255;h=(h[16]&255)<<8|h[17]&255;f.destination_endpoint=l;f.temperature=g/200;f.humidity=k;f.pressure=m;f.accelerationX=
n;f.accelerationY=p;f.accelerationZ=q;f.batteryVoltage=(r>>>5)/1E3+1.6;f.txPower=2*(r&31)-40;f.movementCounter=t;f.measurementSequenceNumber=h;return f},e.yield(navigator.bluetooth.requestLEScan({acceptAllAdvertisements:!0,keepRepeatedDevices:!0}),2);navigator.bluetooth.addEventListener("advertisementreceived",function(h){if(h.device.id==b)if(h=h.manufacturerData.get(1177),!h)Blockly.alert("The device is not a RuuviTag");else if(5==h.getUint8(0)){for(var f=[],l=0;l<h.byteLength;l++)f.push("0x"+("00"+
h.getUint8(l).toString(16)).slice(-2));c(d(f)[a])}});e.jumpToEnd()})};Blast.asyncApiFunctions.push(["getRuuviMmeasurement",getRuuviMmeasurement]);Blockly.Blocks.text_to_speech={init:function(){this.appendValueInput("text").appendField("Text to Speech").setCheck("String");this.setPreviousStatement(!0,null);this.setNextStatement(!0,null);this.setColour(0);this.setTooltip("");this.setHelpUrl("")}};Blast.Toolbox.addBlock("text_to_speech","actions");
