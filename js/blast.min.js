...
Blast.Ui.init=function(){Blast.Ui.uriInput=document.getElementById("loadWorkspace-input");Blast.Ui.runButton=document.getElementById("runButton");Blast.Ui.messageOutputContainer=document.getElementById("msgOutputContainer");Blast.Ui.statusContainer=document.getElementById("statusContainer");if(!window.location.href.includes("mobile.html")){Blast.Ui.tabClick_(Blast.Ui.selected_);Blast.Ui.uriInput.addEventListener("keyup",function(c){13===c.keyCode&&Blast.Storage.load()});Blast.bindClick("loadButton",
Blast.Storage.load);Blast.bindClick("saveButton",Blast.Storage.link);for(var a=$jscomp.makeIterator(Blast.Ui.TABS_),b=a.next();!b.done;b=a.next())b=b.value,Blast.bindClick("tab_"+b,function(c){return function(){Blast.Ui.tabClick_(c)}}(b))}};Blast.Toolbox={};
...
Blast.init=function(){if(window.location.href.includes("mobile.html"))Blast.workspace=new Blockly.Workspace,Blast.Ui.init(),Blast.Ui.setStatus(Blast.status.READY),Blast.Ui.addMessage("Actionblock output will be displayed here");else{var a=document.getElementById("content_area"),b=function(){for(var c=Blast.getBBox_(a),d=$jscomp.makeIterator(Blast.Ui.TABS_),e=d.next();!e.done;e=d.next())e=document.getElementById("content_"+e.value),e.style.top=c.y+"px",e.style.left=c.x+"px",e.style.height=c.height+
"px",e.style.height=2*c.height-e.offsetHeight+"px",e.style.width=c.width+"px",e.style.width=2*c.width-e.offsetWidth+"px";Blast.workspace&&Blast.workspace.getToolbox().width&&(document.getElementById("tab_workspace").style.minWidth=Blast.workspace.getToolbox().width-38+"px")};window.addEventListener("resize",b,!1);Blast.workspace=Blockly.inject("content_workspace",{media:"media/",toolbox:Blast.Toolbox.currentToolbox,zoom:{controls:!0,wheel:!0}});Blast.Toolbox.init();b();Blockly.svgResize(Blast.workspace);
Blast.Ui.init();Blast.Ui.setStatus(Blast.status.READY);Blast.generateCode();Blast.workspace.addChangeListener(function(c){c instanceof Blockly.Events.Ui||(Blast.generateCode(),Blast.States.generateCode(),Blast.Ui.renderContent_())});Blast.Ui.addMessage("Actionblock output will be displayed here");window.setTimeout(Blast.importPrettify,1)}};Blast.highlightBlock=function(a){Blast.workspace.highlightBlock(a);Blast.highlightPause=!0};
Blast.resetUi=function(a){Blast.workspace.highlightBlock(null);Blast.highlightPause=!1;Blast.Ui.setStatus(a)};Blast.generateCode=function(){Blockly.JavaScript.STATEMENT_PREFIX="highlightBlock(%1);\n";Blockly.JavaScript.addReservedWords("highlightBlock");Blast.latestCode="";Blast.latestCode=Blockly.JavaScript.workspaceToCode(Blast.workspace)};Blast.resetInterpreter=function(){Blast.Interpreter=null;Blast.runner_&&(clearTimeout(Blast.runner_),Blast.runner_=null);Blast.Bluetooth.tearDown()};
