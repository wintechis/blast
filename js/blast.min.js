...
g.firstElementChild.textContent;if(d.includes(e.$jscomp$loop$prop$name$47))continue;g=document.createElement("tr");var f=document.createElement("td");f.appendChild(document.createTextNode(e.$jscomp$loop$prop$name$47));var l=document.createElement("td"),h=document.createElement("input");h.setAttribute("type","button");h.setAttribute("id","pairButton-"+e.$jscomp$loop$prop$name$47);h.setAttribute("value","Pair");var k=document.createElement("td"),m=document.createElement("span");m.setAttribute("id",
"pairStatus-"+e.$jscomp$loop$prop$name$47);m.innerHTML="&#x2718;";m.style.color="red";m.style.fontSize="20px";m.style.fontWeight="bold";m.style.marginRight="10px";k.appendChild(m);h.addEventListener("click",function(n){return function(){var p;return $jscomp.asyncExecutePromiseGeneratorProgram(function(q){if(1==q.nextAddress)return"things_webBluetooth"!=n.$jscomp$loop$prop$type$46?("things_webHID"==n.$jscomp$loop$prop$type$46&&(p=[],navigator.hid.requestDevice({filters:p}).then(function(r){0===r.length&&
...
q.jumpToEnd()})}}(e));l.appendChild(h);g.appendChild(f);g.appendChild(l);g.appendChild(k);c.appendChild(g);d.push(e.$jscomp$loop$prop$name$47)}document.getElementById("rc-cancel").addEventListener("click",function(){return Blast.Storage.reconnectCancelHandler_()})}};
...
f=document.createElement("span");f.setAttribute("class","mdc-list-item__text");var l=document.createElement("span");l.setAttribute("class","mdc-list-item__primary-text");l.innerHTML=e.$jscomp$loop$prop$name$50;f.appendChild(l);l=document.createElement("span");l.setAttribute("class","mdc-list-item__secondary-text");l.setAttribute("id","rc-status-"+e.$jscomp$loop$prop$name$50);l.innerHTML="disconnected";f.appendChild(l);g.appendChild(f);g.addEventListener("click",function(h){return function(){var k;
return $jscomp.asyncExecutePromiseGeneratorProgram(function(m){if(1==m.nextAddress)return"things_webBluetooth"!=h.$jscomp$loop$prop$type$49?("things_webHID"==h.$jscomp$loop$prop$type$49&&(k=[],navigator.hid.requestDevice({filters:k}).then(function(n){0===n.length&&Blast.throwError("Connection failed or cancelled by User.");var p=Date.now().toString(36)+Math.random().toString(36).substr(2);Blast.Things.webHidDevices.set(p,n[0]);Blast.Things.addWebHidDevice(p,"");document.getElementById("rc-status-"+
