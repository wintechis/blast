...
Blast.Toolbox.addBlock("read_ruuvi_property","Properties");Blockly.JavaScript.read_ruuvi_property=function(a){var b=a.getFieldValue("measurement");a=Blockly.JavaScript.valueToCode(a,"Thing",Blockly.JavaScript.ORDER_ATOMIC);return["getRuuviProperty('"+b+"', "+a+")",Blockly.JavaScript.ORDER_NONE]};
var getRuuviProperty=function(a,b,c){var d,e;return $jscomp.asyncExecutePromiseGeneratorProgram(function(k){d=function(f){var l={},g=f[0]&255,h=f[1]<<8|f[2]&255;32767<h&&(h-=65535);var m=((f[3]&255)<<8|f[4]&255)/400,n=((f[5]&255)<<8|f[6]&255)+5E4,p=(f[7]<<8|f[8]&255)/1E3,q=(f[9]<<8|f[10]&255)/1E3,t=(f[11]<<8|f[12]&255)/1E3,r=(f[13]&255)<<8|f[14]&255,u=f[15]&255;f=(f[16]&255)<<8|f[17]&255;l.destination_endpoint=g;l.temperature=h/200;l.humidity=m;l.pressure=n;l.accelerationX=p;l.accelerationY=q;l.accelerationZ=
t;l.batteryVoltage=(r>>>5)/1E3+1.6;l.txPower=2*(r&31)-40;l.movementCounter=u;l.measurementSequenceNumber=f;return l};e=function(f){if(f.device.id==b&&(f=f.manufacturerData.get(1177))&&5==f.getUint8(0)){for(var l=[],g=0;g<f.byteLength;g++)l.push("0x"+("00"+f.getUint8(g).toString(16)).slice(-2));f=d(l);c(f[a])}};Blast.Bluetooth.addEventListener("advertisementreceived",e);Blast.Bluetooth.requestLEScan();k.jumpToEnd()})};Blast.asyncApiFunctions.push(["getRuuviProperty",getRuuviProperty]);Blockly.Blocks.streamdeck_buttons={init:function(){this.appendValueInput("id").setCheck("Thing").appendField("Streamdeck mini");this.appendDummyInput().appendField("on button press:");this.appendDummyInput().setAlign(Blockly.ALIGN_CENTRE).appendField(new Blockly.FieldCheckbox("FALSE"),"button1").appendField(new Blockly.FieldCheckbox("FALSE"),"button2").appendField(new Blockly.FieldCheckbox("FALSE"),"button3");this.appendDummyInput().setAlign(Blockly.ALIGN_CENTRE).appendField(new Blockly.FieldCheckbox("FALSE"),
