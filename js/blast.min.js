...
n.$jscomp$loop$prop$name$42).style.color="green";n.$jscomp$loop$prop$block$43.firstElementChild.textContent=u;console.log(n.$jscomp$loop$prop$block$43);Blast.Storage.allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)}))}).catch(function(t){wHidLog("Argh! "+t)})),r.jumpTo(0)):r.yield(Blast.Bluetooth.requestDevice(),3);q=r.yieldResult;document.getElementById("pairStatus-"+
n.$jscomp$loop$prop$name$42).innerHTML="&#x2714;";document.getElementById("pairStatus-"+n.$jscomp$loop$prop$name$42).style.color="green";n.$jscomp$loop$prop$block$43.firstElementChild.textContent=q.id;Blast.Storage.allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").addEventListener("click",function(){return Blast.Storage.reconnectDoneHandler_(a)}));r.jumpToEnd()})}}(e));l.appendChild(h);g.appendChild(f);g.appendChild(l);g.appendChild(k);c.appendChild(g);
d.push(e.$jscomp$loop$prop$name$42)}document.getElementById("rc-cancel").addEventListener("click",function(){return Blast.Storage.reconnectCancelHandler_()})}};
...
Blast.Bluetooth.requestDevice=function(a,b){return $jscomp.asyncExecutePromiseGeneratorProgram(function(c){navigator.bluetooth&&(a||(a={acceptAllDevices:!0},a.optionalServices=Blast.Bluetooth.optionalServices),navigator.bluetooth.requestDevice(a).then(function(d){Blast.Things.addWebBluetoothDevice(d.id,b||d.name);Blast.workspace.refreshToolboxSelection();return d}).catch(function(d){Blast.throwError(d)}));c.jumpToEnd()})};
...
n)),Blast.Ui.setStatus(Blast.status.ERROR),Blast.resetInterpreter(),console.error(n)}};m()}};Blast.deviceEventHandlers.push({device:e,type:d,fn:f});e.addEventListener(d,f);l.jumpToEnd()}})};Blast.apiFunctions.push(["handleStreamdeck",handleStreamdeck]);
