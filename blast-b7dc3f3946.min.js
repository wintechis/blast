...
a.next()){c=c.value;var d=Blockly.utils.xml.createElement("block");d.setAttribute("type","uri");var f=Blockly.utils.xml.createElement("field");f.setAttribute("name","URI");f.textContent=c;d.appendChild(f);b.push(d)}return b};Blast.Things.thingsFlyoutCategory=module$contents$Blast$Things_flyoutCategory;
...
createWebHidButtonHandler=function(){module$contents$Blast$Ui_addToLog("Requesting webHID device...","HID");navigator.hid.requestDevice({filters:[]}).then(function(a){if(0===a.length)Blast.throwError("Connection failed or cancelled by User.");else{var b=Date.now().toString(36)+Math.random().toString(36).substr(2);module$contents$Blast$Things_addWebHidDevice(b,a[0].productName,a[0]);Blast.workspace.refreshToolboxSelection();module$contents$Blast$Ui_addToLog("Connected","HID",a[0].productName)}}).catch(function(a){Blast.throwError(a)})};
var module$contents$Blast$Things_addWebHidDevice=function(a,b,c){var d=function(f,h){Blockly.Variables.promptName("Connection established! Now give your device a name.",f,function(g){if(g)if(module$contents$Blast$Things_webHidNames.has(g)){var l="Name %1 already exists".replace("%1",g);Blockly.alert(l,function(){d(g,h)})}else module$contents$Blast$Things_webHidDevices.set(uid,c),module$contents$Blast$Things_webHidNames.set(g,h);else Blockly.alert("Name cannot be empty",function(){d(g,h)})})};d(b,
a)};Blast.Things.addWebHidDevice=module$contents$Blast$Things_addWebHidDevice;Blast.Storage={};
...
var module$contents$Blast$Storage_resetFileInput=function(){var a=document.getElementById("file-selector");a&&(a.value="");module$contents$Blast$Storage_filename="BLAST.xml"},module$contents$Blast$Storage_retrieveXML_=function(a){return $jscomp.asyncExecutePromiseGeneratorProgram(function(b){Blockly.hideChaff();Blast.resetInterpreter();Blast.resetUi(Blast.status.READY);module$contents$Blast$Things_resetThings();module$contents$Blast$Storage_resetFileInput();fetch(a).then(function(c){if(c.ok)return c.text();
Blast.throwError(module$contents$Blast$Storage_NOT_FOUND_ERROR.replace("%1",a))}).then(module$contents$Blast$Storage_loadXML);b.jumpToEnd()})},module$contents$Blast$Storage_loadXML=function(a){Blast.workspace.clear();try{var b=Blockly.Xml.textToDom(a)}catch(c){Blast.throwError(module$contents$Blast$Storage_XML_ERROR+"\nXML: "+b);return}b.querySelector('block[type="things_webBluetooth"]')||b.querySelector('block[type="things_webHID"]')?(module$contents$Blast$Storage_generatePairButtons(b),document.getElementById("rcModal")&&
(document.getElementById("rcModal").style.display="block")):(Blockly.Xml.domToWorkspace(b,Blast.workspace),module$contents$Blast$Storage_monitorChanges_(Blast.workspace))},module$contents$Blast$Storage_generatePairButtons=function(a){window.location.href.includes("mobile")?(window.app.openReconnectDialog(),module$contents$Blast$Storage_generatePairButtonsMobile_(a)):module$contents$Blast$Storage_generatePairButtonsDesktop_(a)},module$contents$Blast$Storage_generatePairButtonsDesktop_=function(a){for(var b=
a.querySelectorAll("block"),c=document.getElementById("rc-tbody");c.firstChild;)c.removeChild(c.firstChild);var d=[],f={};b=$jscomp.makeIterator(b);for(var h=b.next();!h.done;f={$jscomp$loop$prop$type$74:f.$jscomp$loop$prop$type$74,$jscomp$loop$prop$name$75:f.$jscomp$loop$prop$name$75,$jscomp$loop$prop$block$76:f.$jscomp$loop$prop$block$76},h=b.next()){f.$jscomp$loop$prop$block$76=h.value;f.$jscomp$loop$prop$type$74=f.$jscomp$loop$prop$block$76.getAttribute("type");if("things_webBluetooth"==f.$jscomp$loop$prop$type$74||
"things_webHID"==f.$jscomp$loop$prop$type$74){f.$jscomp$loop$prop$name$75=f.$jscomp$loop$prop$block$76.firstElementChild.textContent;if(d.includes(f.$jscomp$loop$prop$name$75))continue;h=document.createElement("tr");var g=document.createElement("td");g.appendChild(document.createTextNode(f.$jscomp$loop$prop$name$75));var l=document.createElement("td"),k=document.createElement("input");k.setAttribute("type","button");k.setAttribute("id","pairButton-"+f.$jscomp$loop$prop$name$75);k.setAttribute("value",
"Pair");var m=document.createElement("td"),n=document.createElement("span");n.setAttribute("id","pairStatus-"+f.$jscomp$loop$prop$name$75);n.innerHTML="&#x2718;";n.style.color="red";n.style.fontSize="20px";n.style.fontWeight="bold";n.style.marginRight="10px";m.appendChild(n);k.addEventListener("click",function(p){return function(){var q,r;return $jscomp.asyncExecutePromiseGeneratorProgram(function(t){if(1==t.nextAddress)return"things_webBluetooth"!=p.$jscomp$loop$prop$type$74?("things_webHID"==p.$jscomp$loop$prop$type$74&&
(q=[],navigator.hid.requestDevice({filters:q}).then(function(v){0===v.length&&Blast.throwError("Connection failed or cancelled by User.");var w=Date.now().toString(36)+Math.random().toString(36).substr(2);module$contents$Blast$Things_addWebHidDevice(w,p.$jscomp$loop$prop$name$75,v[0]);document.getElementById("pairStatus-"+p.$jscomp$loop$prop$name$75).innerHTML="&#x2714;";document.getElementById("pairStatus-"+p.$jscomp$loop$prop$name$75).style.color="green";p.$jscomp$loop$prop$block$76.firstElementChild.textContent=
w;module$contents$Blast$Storage_allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").getAttribute("data-hasEvent")||(document.getElementById("rc-done").setAttribute("data-hasEvent",!0),document.getElementById("rc-done").addEventListener("click",function(){return module$contents$Blast$Storage_reconnectDoneHandler_(a)})))}).catch(function(v){Blast.throwError("Connection failed or cancelled by User.");console.error(v)})),t.jumpTo(0)):t.yield(Blast.Bluetooth.requestDevice(),
3);r=t.yieldResult;document.getElementById("pairStatus-"+p.$jscomp$loop$prop$name$75).innerHTML="&#x2714;";document.getElementById("pairStatus-"+p.$jscomp$loop$prop$name$75).style.color="green";p.$jscomp$loop$prop$block$76.firstElementChild.textContent=r.id;module$contents$Blast$Storage_allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").getAttribute("data-hasEvent")||(document.getElementById("rc-done").setAttribute("data-hasEvent",!0),document.getElementById("rc-done").addEventListener("click",
function(){return module$contents$Blast$Storage_reconnectDoneHandler_(a)})));t.jumpToEnd()})}}(f));l.appendChild(k);h.appendChild(g);h.appendChild(l);h.appendChild(m);c.appendChild(h);d.push(f.$jscomp$loop$prop$name$75)}document.getElementById("rc-cancel").addEventListener("click",function(){return module$contents$Blast$Storage_reconnectCancelHandler_()})}},module$contents$Blast$Storage_generatePairButtonsMobile_=function(a){for(var b=a.querySelectorAll("block"),c=document.getElementById("rc-list");c.firstChild;)c.removeChild(c.firstChild);
var d=[],f={};b=$jscomp.makeIterator(b);for(var h=b.next();!h.done;f={$jscomp$loop$prop$type$78:f.$jscomp$loop$prop$type$78,$jscomp$loop$prop$name$79:f.$jscomp$loop$prop$name$79,$jscomp$loop$prop$block$80:f.$jscomp$loop$prop$block$80},h=b.next())if(f.$jscomp$loop$prop$block$80=h.value,f.$jscomp$loop$prop$type$78=f.$jscomp$loop$prop$block$80.getAttribute("type"),"things_webBluetooth"==f.$jscomp$loop$prop$type$78||"things_webHID"==f.$jscomp$loop$prop$type$78)if(f.$jscomp$loop$prop$name$79=f.$jscomp$loop$prop$block$80.firstElementChild.textContent,
!d.includes(f.$jscomp$loop$prop$name$79)){h=document.createElement("li");h.setAttribute("id","rc-item-"+f.$jscomp$loop$prop$name$79);h.setAttribute("class","mdc-list-item");var g=document.createElement("span");g.setAttribute("id","rc-icon-"+f.$jscomp$loop$prop$name$79);g.setAttribute("class","mdc-list-item__graphic material-icons");g.setAttribute("aria-hidden","true");g.innerHTML="things_webBluetooth"==f.$jscomp$loop$prop$type$78?"bluetooth_disabled":"usb_off";h.appendChild(g);g=document.createElement("span");
g.setAttribute("class","mdc-list-item__text");var l=document.createElement("span");l.setAttribute("class","mdc-list-item__primary-text");l.innerHTML=f.$jscomp$loop$prop$name$79;g.appendChild(l);l=document.createElement("span");l.setAttribute("class","mdc-list-item__secondary-text");l.setAttribute("id","rc-status-"+f.$jscomp$loop$prop$name$79);l.innerHTML="disconnected";g.appendChild(l);h.appendChild(g);h.addEventListener("click",function(k){return function(){var m,n;return $jscomp.asyncExecutePromiseGeneratorProgram(function(p){if(1==
p.nextAddress)return"things_webBluetooth"!=k.$jscomp$loop$prop$type$78?("things_webHID"==k.$jscomp$loop$prop$type$78&&(m=[],navigator.hid.requestDevice({filters:m}).then(function(q){0===q.length&&Blast.throwError("Connection failed or cancelled by User.");var r=Date.now().toString(36)+Math.random().toString(36).substr(2);module$contents$Blast$Things_addWebHidDevice(r,k.$jscomp$loop$prop$name$79,q[0]);document.getElementById("rc-status-"+k.$jscomp$loop$prop$name$79).innerHTML="connected";document.getElementById("rc-icon-"+
k.$jscomp$loop$prop$name$79).innerHTML="usb";document.getElementById("rc-icon-"+k.$jscomp$loop$prop$name$79).style.color="#0d30b1";k.$jscomp$loop$prop$block$80.firstElementChild.textContent=r;module$contents$Blast$Storage_allConnectedDesktop_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").getAttribute("data-hasEvent")||(document.getElementById("rc-done").setAttribute("data-hasEvent",!0),document.getElementById("rc-done").addEventListener("click",function(){return module$contents$Blast$Storage_reconnectDoneHandler_(a)})))}).catch(function(q){Blast.throwError("Connection failed or cancelled by User.");
console.error(q)})),p.jumpTo(0)):p.yield(Blast.Bluetooth.requestDevice(),3);n=p.yieldResult;module$contents$Blast$Things_addWebBluetoothDevice(n.id,k.$jscomp$loop$prop$name$79);document.getElementById("rc-status-"+k.$jscomp$loop$prop$name$79).innerHTML="connected";document.getElementById("rc-icon-"+k.$jscomp$loop$prop$name$79).innerHTML="bluetooth";document.getElementById("rc-icon-"+k.$jscomp$loop$prop$name$79).style.color="#0d30b1";k.$jscomp$loop$prop$block$80.firstElementChild.textContent=n.id;
module$contents$Blast$Storage_allConnectedMobile_()&&(document.getElementById("rc-done").disabled=!1,document.getElementById("rc-done").getAttribute("data-hasEvent")||(document.getElementById("rc-done").setAttribute("data-hasEvent",!0),document.getElementById("rc-done").addEventListener("click",function(){return module$contents$Blast$Storage_reconnectDoneHandler_(a)})));p.jumpToEnd()})}}(f));c.appendChild(h);d.push(f.$jscomp$loop$prop$name$79)}window.app.initLists()},module$contents$Blast$Storage_allConnectedDesktop_=
function(){var a=document.getElementById("rc-tbody").querySelectorAll("tr");a=$jscomp.makeIterator(a);for(var b=a.next();!b.done;b=a.next())if("\u2718"==document.getElementById("pairStatus-"+b.value.firstElementChild.textContent).innerHTML)return!1;return!0},module$contents$Blast$Storage_allConnectedMobile_=function(){var a=document.getElementById("rc-list").querySelectorAll("[id=rc-status-]");a=$jscomp.makeIterator(a);for(var b=a.next();!b.done;b=a.next())if("disconnected"==document.getElementById("rc-status-"+
b.value.textContent).innerHTML)return!1;return!0},module$contents$Blast$Storage_reconnectDoneHandler_=function(a){window.location.href.includes("mobile")?document.getElementById("rc-dialog").style.display="none":document.getElementById("rcModal").style.display="none";Blockly.Xml.domToWorkspace(a,Blast.workspace);module$contents$Blast$Storage_monitorChanges_(Blast.workspace)},module$contents$Blast$Storage_reconnectCancelHandler_=function(){document.getElementById("rcModal").style.display="none"},module$contents$Blast$Storage_monitorChanges_=
function(a){function b(){var f=Blockly.Xml.workspaceToDom(a);f=Blockly.Xml.domToText(f);d!=f&&(window.location.hash="",a.removeChangeListener(b))}var c=Blockly.Xml.workspaceToDom(a),d=Blockly.Xml.domToText(c);a.addChangeListener(b)},module$contents$Blast$Storage_generatePath=function(){for(var a=[],b=0;12>b;b++)a.push("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(62*Math.random())));return a.join("")};
window.addEventListener("load",function(){var a=window.location.hash;if(a){a=a.substring(1);try{module$contents$Blast$Storage_retrieveXML_(a)}catch(b){Blast.throwError("Could not load file."),console.error(b)}}});Blast.Ui={};
...
d.appendChild(f);f=function(){document.getElementById("debugModal").style.display="block";var g=document.getElementById("debug-tbody");g.innerHTML="";var l=document.createElement("tr");l.innerHTML="<td>BLAST</td><td>"+rev+"</td>";g.appendChild(l);for(var k in navigator)if("string"===typeof navigator[k]){l=document.createElement("tr");var m=document.createElement("td"),n=document.createElement("td");m.innerHTML=k;n.innerHTML=navigator[k];l.appendChild(m);l.appendChild(n);g.appendChild(l)}};if("error"===
...
Blast.throwError=function(a){a||(a="Error executing program - See console for details.");module$contents$Blast$Ui_addMessage(a,"error");module$contents$Blast$Ui_setStatus(Blast.status.ERROR);Blast.resetInterpreter();module$contents$Blast$Ui_addMessage("Execution stopped","info")};Blast.handleFetchErrors=function(a,b){a.ok||(b?Blast.throwError(b):Blast.throwError("Error processing HTTP Request."));return a};window.addEventListener("load",Blast.init);Blast.Bluetooth={};Blast.Bluetooth.optionalServices=[];Blast.Bluetooth.scanBlocks=[];Blast.Bluetooth.LEScanResults={};Blast.Bluetooth.isLEScanRunning=!1;Blast.Bluetooth.eventListeners=[];Blast.Bluetooth.charEventListeners={};function hexStringToArrayBuffer(a){a=a.replace(/^0x/,"");0!=a.length%2&&(a="0"+a);var b=a.match(/[G-Z\s]/i);b&&console.log("WARNING: found non-hex characters",b);a=a.match(/[\dA-F]{2}/gi).map(function(c){return parseInt(c,16)});return(new Uint8Array(a)).buffer}
...
"Bluetooth",c.id),d=b||c.name,module$contents$Blast$Things_addWebBluetoothDevice(c.id,d),Blast.workspace.refreshToolboxSelection(),h.return(c);f=h.enterCatchBlock();Blast.throwError(f);h.jumpToEnd()})};
...
break;case "01":n="https://www.";break;case "02":n="http://";break;case "03":n="https://";break;default:Blast.throwError("Eddystone URL scheme is not valid.");return}var p=".com/ .org/ .edu/ .net/ .info/ .biz/ .gov/ .com .org .edu .net .info .biz .gov".split(" ");k=k.substring(6,k.length);for(var q=0;q<k.length;q+=2){var r=parseInt(k.substring(q,q+2),16);n=r<p.length?n+p[r]:n+String.fromCharCode(r)}return{frameType:"URL",txPower:m,url:n}},d=function(k){var m=parseInt(k.substring(4,8),16),n=parseInt(k.substring(8,
10),16)+parseInt(k.substring(10,12),16)/100,p=parseInt(k.substring(12,20),16);k=parseInt(k.substring(20,28),16);return{frameType:"TLM",batteryVoltage:m,beaconTemperature:n,pduCount:p,timeSinceReboot:k}},l.yield(Blast.Bluetooth.gatt_read_hex(a,Blast.Bluetooth.Eddystone.UUIDS.CONFIG_SERVICE,Blast.Bluetooth.Eddystone.UUIDS.ADV_SLOT_DATA_CHARACTERISTIC),2);f=l.yieldResult;module$contents$Blast$Ui_addToLog("Got Eddystone advertising data: <code>"+f+"</code>","Eddystone",a);h=f.substring(0,2);switch(h){case "00":return g=
b(f),l.return(g.namespace+g.instance);case "10":return g=c(f),l.return(g.url);case "20":return g=d(f),l.return(g.beaconTemperature);case "30":return Blast.throwError("EID frame type is not supported by blast."),l.return();default:return Blast.throwError("Eddystone frame type is not valid."),l.return()}})};
...
break}for(;p<l.length;){k=p;for(r=0;r<m.length;r++)if(l.startsWith(m[r],p)){n.push(r);p+=m[r].length;break}k===p&&(n.push(q.encode(l[p])[0]),p++)}if(18<n.length)Blast.throwError("URL is too long.");else return n.splice(0,0,16),new Uint8Array(n)};if("URL"===b)f=d(c);else if("UID"===b){if(!/^[0-9A-Fa-f]{32}$/.test(c))return Blast.throwError("Eddystone UID must be 32 hexadecimal characters."),g.return();f="0x00"+c}return g.yield(Blast.Bluetooth.gatt_writeWithResponse(a,Blast.Bluetooth.Eddystone.UUIDS.CONFIG_SERVICE,
...
Blockly.Blocks.things_webHID={init:function(){this.appendDummyInput().appendField("HID device").appendField(new Blockly.FieldDropdown(module$contents$Blast$Things_getWebHIDDevices),"id");this.setOutput(!0,"Thing");this.setColour(60);this.setTooltip("");this.setHelpUrl("")}};Blockly.JavaScript.repeat=Blockly.JavaScript.controls_repeat_ext;Blockly.JavaScript.while_until=Blockly.JavaScript.controls_whileUntil;Blockly.JavaScript["for"]=Blockly.JavaScript.controls_for;Blockly.JavaScript.break_continue=Blockly.JavaScript.controls_flow_statements;Blockly.JavaScript.conditional_statement=Blockly.JavaScript.controls_if;Blockly.JavaScript.wait_seconds=function(a){return"waitForSeconds("+(Blockly.JavaScript.valueToCode(a,"SECONDS",Blockly.JavaScript.ORDER_ATOMIC)||0)+");\n"};
...
var sendHttpRequest=function(a,b,c,d,f,h){var g,l,k,m,n;return $jscomp.asyncExecutePromiseGeneratorProgram(function(p){switch(p.nextAddress){case 1:return null!=a&&void 0!=a&&""!=a||Blast.throwError("URI input of HttpRequest blocks must not be empty"),g=JSON.parse(c),l={method:b,headers:new Headers(g)},"GET"!==b&&d&&(l.body=d),p.setCatchFinallyBlocks(2),p.yield(fetch(a,l),4);case 4:return k=p.yieldResult,k.ok?"status"==f?(h(k.status),p.return()):p.yield(k.text(),5):(Blast.throwError("Failed to get "+
a+", Error: "+k.status+" "+k.statusText),p.return());case 5:m=p.yieldResult;h(m);p.leaveTryBlock(0);break;case 2:n=p.enterCatchBlock(),Blast.throwError("Failed to get "+a+", Error: "+n.message),p.jumpToEnd()}})};Blast.asyncApiFunctions.push(["sendHttpRequest",sendHttpRequest]);
...
u.leaveTryBlock(3);break;case 2:l=u.enterCatchBlock(),Blast.throwError("Failed to get "+a+", Error: "+l.message);case 3:if("boolean"===typeof f)return d(f),u.return();k=Array(f.length);m=$jscomp.makeIterator(f);for(n=m.next();!n.done;n=m.next()){p=n.value;q=Array(Object.keys(p).length);r=$jscomp.makeIterator(Object.values(p));for(t=r.next();!t.done;t=r.next())v=t.value,q.push(v.value);k.push(q)}w=Blast.Interpreter.nativeToPseudo(k);d(w);u.jumpToEnd()}})};
...
m){b.onloadedmetadata=function(){f.width=b.videoWidth;f.height=b.videoHeight;h.drawImage(b,0,0,f.width,f.height);k()};video.onerror=function(n){Blast.throwError("Error trying to capture image from camera. See console for details");console.error(n);m(n)}}),3);g=f.toDataURL("image/png");b.srcObject.getTracks().forEach(function(k){return k.stop()});b.remove();f.remove();a(g);l.jumpToEnd()})};Blast.asyncApiFunctions.push(["captureImage",captureImage]);
...
var getRSSIWb=function(a,b){var c,d,f,h,g,l;return $jscomp.asyncExecutePromiseGeneratorProgram(function(k){if(1==k.nextAddress)return k.yield(navigator.bluetooth.getDevices(),2);c=k.yieldResult;d=null;f=$jscomp.makeIterator(c);for(h=f.next();!h.done;h=f.next())if(g=h.value,g.id===a){d=g;break}null==d&&Blast.throwError("Error pairing with Bluetooth device.");l=new AbortController;d.addEventListener("advertisementreceived",function(m){return $jscomp.asyncExecutePromiseGeneratorProgram(function(n){l.abort();
b(m.rssi);n.jumpToEnd()})});return k.yield(d.watchAdvertisements({signal:l.signal}),0)})};Blast.asyncApiFunctions.push(["getRSSIWb",getRSSIWb]);
...
var readJoyConProperty=function(a,b,c,d,f,h){var g,l,k;return $jscomp.asyncExecutePromiseGeneratorProgram(function(m){switch(m.nextAddress){case 1:if(!a)return Blast.throwError("No Joy-Con block set."),h(),m.return();g=module$contents$Blast$Things_webHidDevices.get(a);if(!g)return Blast.throwError("Connected device is not a HID device.\nMake sure you are connecting the JoyCon via webHID"),h(),m.return();if(g.opened){m.jumpTo(2);break}m.setCatchFinallyBlocks(3);return m.yield(g.open(),5);case 5:m.leaveTryBlock(2);
break;case 3:m.enterCatchBlock(),Blast.throwError("Failed to open device, your browser or OS probably doesn't support webHID.");case 2:if(1406!==g.vendorId||8198!==g.productId&&8199!==g.productId)return Blast.throwError("The connected device is not a Joy-Con."),h(),m.return();8198===g.productId?l=new JoyConLeft(g):8199===g.productId&&(l=new JoyConRight(g));return m.yield(l.open(),6);case 6:return m.yield(l.enableStandardFullMode(),7);case 7:return m.yield(l.enableIMUMode(),8);case 8:k=function(n){var p;
return $jscomp.asyncExecutePromiseGeneratorProgram(function(q){p=n.detail;if(!p||!p.actualOrientation)return q.return();module$contents$Blast$Ui_addToLog("Received <code>hidinput</code> event from Joy-Con: <code>"+JSON.stringify(p)+"</code>","hid",g.productname);module$contents$Blast$Ui_addToLog("Removing <code>hidinput</code> event listener","hid",g.productname);l.removeEventListener("hidinput",k);""!==d?"accelerometers"===b?h(p[b][c][d].acc):"gyroscopes"===b?h(p[b][c][d][f]):h(p[b][c][d]):h(p[b][c]);
q.jumpToEnd()})};if(l.eventListenerAttached){m.jumpTo(0);break}return m.yield(l.open(),10);case 10:return m.yield(l.enableStandardFullMode(),11);case 11:return m.yield(l.enableIMUMode(),12);case 12:return m.yield(l.enableVibration(),13);case 13:module$contents$Blast$Ui_addToLog("Adding <code>hidinput</code> event listener","hid",g.productname),l.addEventListener("hidinput",k),l.eventListenerAttached=!0,m.jumpToEnd()}})};Blast.asyncApiFunctions.push(["readJoyConProperty",readJoyConProperty]);
...
var handleStreamdeck=function(a,b,c,d){var f,h,g,l,k;return $jscomp.asyncExecutePromiseGeneratorProgram(function(m){switch(m.nextAddress){case 1:if(null===a)return Blast.throwError("No streamdeck block set."),m.return();f=module$contents$Blast$Things_webHidDevices.get(a);if(!f)return Blast.throwError("Connected device is not a HID device.\nMake sure you are connecting the Streamdeck via webHID"),m.return();m.setCatchFinallyBlocks(2);return m.yield(StreamDeck.openDevice(f),4);case 4:h=m.yieldResult;
m.leaveTryBlock(3);break;case 2:g=m.enterCatchBlock();if("InvalidStateError"!=g.name)return Blast.throwError(g),m.return();f.close();return m.yield(StreamDeck.openDevice(f),6);case 6:h=m.yieldResult;case 3:for(k=0;k<b.length;k++)if("1"===b.charAt(k)){l=k;break}if(void 0===l)return m.return();h.on(c,function(n){module$contents$Blast$Ui_addToLog("Received <code>"+c+"</code> event on button <code>"+n+"</code>","hid",f.productName);if(n===l){Blast.Interrupted=!0;var p=new Interpreter("");p.stateStack[0].scope=
Blast.Interpreter.globalScope;p.appendCode(d);var q=function(){try{p.step()?setTimeout(q,5):Blast.Interrupted=!1}catch(r){Blast.throwError("Error executing program:\n "+e),console.error(r)}};q()}});Blast.cleanUpFunctions.push(function(){module$contents$Blast$Ui_addToLog("Removing all listeners","hid",f.productName);h.close();h.removeAllListeners()});m.jumpToEnd()}})};Blast.apiFunctions.push(["handleStreamdeck",handleStreamdeck]);
var streamdeckColorButtons=function(a,b,c,d){var f,h,g,l,k,m,n;return $jscomp.asyncExecutePromiseGeneratorProgram(function(p){switch(p.nextAddress){case 1:if(null===a)return Blast.throwError("No streamdeck block set."),d(),p.return();f=module$contents$Blast$Things_webHidDevices.get(a);if(!f)return Blast.throwError("Connected device is not a HID device.\nMake sure you are connecting the Streamdeck via webHID"),d(),p.return();p.setCatchFinallyBlocks(2);return p.yield(StreamDeck.openDevice(f),4);case 4:h=
p.yieldResult;p.leaveTryBlock(3);break;case 2:g=p.enterCatchBlock();if("InvalidStateError"!=g.name)return Blast.throwError(g),d(),p.return();f.close();return p.yield(StreamDeck.openDevice(f),6);case 6:h=p.yieldResult;case 3:l=parseInt(c.substring(1,3),16),k=parseInt(c.substring(3,5),16),m=parseInt(c.substring(5,7),16),n=0;case 7:if(!(n<b.length)){p.jumpTo(9);break}if("1"!==b.charAt(n)){p.jumpTo(8);break}module$contents$Blast$Ui_addToLog("Invoke <code>fillKeyColor</code> with value <code>"+[n,l,k,
m].toString()+"</code>","hid",f.productName);return p.yield(h.fillKeyColor(n,l,k,m),11);case 11:module$contents$Blast$Ui_addToLog("Finished <code>fillKeyColor</code>","hid",f.productName);case 8:n++;p.jumpTo(7);break;case 9:d(),p.jumpToEnd()}})};Blast.asyncApiFunctions.push(["streamdeckColorButtons",streamdeckColorButtons]);
...
var streamdeckWriteOnButtons=function(a,b,c,d){var f,h,g,l,k,m,n,p;return $jscomp.asyncExecutePromiseGeneratorProgram(function(q){switch(q.nextAddress){case 1:if(null===a)return Blast.throwError("No streamdeck block set."),d(),q.return();f=module$contents$Blast$Things_webHidDevices.get(a);if(!f)return Blast.throwError("Connected device is not a HID device.\nMake sure you are connecting the Streamdeck via webHID"),d(),q.return();q.setCatchFinallyBlocks(2);return q.yield(StreamDeck.openDevice(f),4);
case 4:h=q.yieldResult;q.leaveTryBlock(3);break;case 2:g=q.enterCatchBlock();if("InvalidStateError"!=g.name)return Blast.throwError(g),d(),q.return();f.close();return q.yield(StreamDeck.openDevice(f),6);case 6:h=q.yieldResult;case 3:l=[];k=document.createElement("canvas");k.width=h.ICON_SIZE;k.height=h.ICON_SIZE;m=k.getContext("2d");m.save();m.clearRect(0,0,k.width,k.height);m.font=.8*k.height+"px Arial";m.strokeStyle="blue";m.lineWidth=1;m.strokeText(c.toString(),8,60,.8*k.width);m.fillStyle="white";
m.fillText(c.toString(),8,60,.8*k.width);n=m.getImageData(0,0,k.width,k.height);for(p=0;p<b.length;p++)"1"===b.charAt(p)&&(module$contents$Blast$Ui_addToLog("Invoke <code>fillKeyImageData</code> with value <code>"+[p,n].toString()+"</code>","hid",f.productName),l.push(h.fillKeyBuffer(p,buffer.Buffer.from(n.data),{format:"rgba"})),module$contents$Blast$Ui_addToLog("Finished <code>fillKeyImageData</code>","hid",f.productName));m.restore();return q.yield(Promise.all(l),7);case 7:d(),q.jumpToEnd()}})};
...
var webSpeech=function(a,b){var c,d,f;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){c=Blast.workspace.getBlockById(a);d=c.recognition;d.continuous=!1;d.lang="en-US";f="";d.onresult=function(g){for(var l=g.resultIndex;l<g.results.length;++l)g.results[l].isFinal&&(f+=g.results[l][0].transcript)};d.onend=function(){b(f)};d.start();h.jumpToEnd()})};Blast.asyncApiFunctions.push(["webSpeech",webSpeech]);
