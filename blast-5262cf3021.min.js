...
d=f(d);m.setCatchFinallyBlocks(3);return m.yield(g.writeValueWithoutResponse(d),5);case 5:m.leaveTryBlock(4);break;case 3:h=m.enterCatchBlock(),Blast.throwError("Error writing to Bluetooth device.\nMake sure the device is compatible with the connected block."),console.error(h);case 4:return m.yield(Blast.Bluetooth.disconnect(a),0)}})};
Blast.Bluetooth.getPrimaryService=function(a,b){var c,d,f;return $jscomp.asyncExecutePromiseGeneratorProgram(function(g){switch(g.nextAddress){case 1:return g.yield(Blast.Bluetooth.connect(a,b),2);case 2:return c=g.yieldResult,g.setCatchFinallyBlocks(3),g.yield(c.getPrimaryService(b),5);case 5:d=g.yieldResult;g.leaveTryBlock(4);break;case 3:f=g.enterCatchBlock(),Blast.throwError("The device is not compatible with the connected block."),console.error(f);case 4:return g.return(d)}})};
Blast.Bluetooth.getCharacteristic=function(a,b,c){var d,f,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){switch(h.nextAddress){case 1:return h.yield(Blast.Bluetooth.getPrimaryService(a,b),2);case 2:d=h.yieldResult;if(!d)return h.return();h.setCatchFinallyBlocks(3);return h.yield(d.getCharacteristic(c),5);case 5:f=h.yieldResult;h.leaveTryBlock(4);break;case 3:g=h.enterCatchBlock(),Blast.throwError("The device is not compatible with the connected block."),console.error(g);case 4:return h.return(f)}})};
Blast.Bluetooth.gatt_read=function(a,b,c){var d,f;return $jscomp.asyncExecutePromiseGeneratorProgram(function(g){if(1==g.nextAddress)return d=Blast.Bluetooth.getCharacteristic(a,b,c),g.setCatchFinallyBlocks(2),g.yield(d.readValue(),4);if(2!=g.nextAddress)return g.return(g.yieldResult);f=g.enterCatchBlock();Blast.throwError("Error reading from Bluetooth device "+a);console.error(f);g.jumpToEnd()})};
...
Blast.Bluetooth.gatt_subscribe=function(a,b,c,d){var f,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){switch(h.nextAddress){case 1:return h.yield(Blast.Bluetooth.getCharacteristic(a,b,c),2);case 2:return f=h.yieldResult,f.addEventListener("characteristicvaluechanged",d),Blast.Bluetooth.charEventListeners[c]=[f,"characteristicvaluechanged",d],h.setCatchFinallyBlocks(3),h.yield(f.startNotifications(),5);case 5:h.leaveTryBlock(0);break;case 3:g=h.enterCatchBlock(),Blast.throwError("Error subscribing to Bluetooth device "+
a),console.error(g),h.jumpToEnd()}})};
...
3E4);a.jumpToEnd()})};Blast.Bluetooth.stopLEScan=function(){Blast.Bluetooth.isLEScanRunning&&(navigator.bluetooth.stopLEScan(),Blast.Bluetooth.isLEScanRunning=!1)};Blast.Bluetooth.cacheLEScanResults=function(){console.log("Caching LE Scan results.");Blast.Bluetooth.addEventListener("advertisementreceived",function(a){console.log("Caching LE Scan results.");console.log(a);var b=a.device.id;Blast.Bluetooth.LEScanResults[b]||(Blast.Bluetooth.LEScanResults[b]=[]);Blast.Bluetooth.LEScanResults[b].unshift(a)})};
