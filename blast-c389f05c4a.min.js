...
m.yield(new Promise(function(k,l){b.onloadedmetadata=function(){f.width=b.videoWidth;f.height=b.videoHeight;h.drawImage(b,0,0,f.width,f.height);k()}}),3);g=f.toDataURL("image/png");b.srcObject.getTracks().forEach(function(k){return k.stop()});b.remove();f.remove();a(g);m.jumpToEnd()})};Blast.asyncApiFunctions.push(["captureImage",captureImage]);Blockly.JavaScript.get_signal_strength_wb=function(a){return["getRSSIWb("+Blockly.JavaScript.valueToCode(a,"Thing",Blockly.JavaScript.ORDER_NONE)+")",Blockly.JavaScript.ORDER_NONE]};
